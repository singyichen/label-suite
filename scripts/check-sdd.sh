...
EOF
        ...
        while IFS= read -r task_line; do
            ...
            if printf '%s\n' "$task_line" | grep -Fq 'Exception:'; then
                ...
                if [ "$exception" = invalid ] || [ "$exception_fields" -ne 3 ] || printf '%s\n' "$task_line" | grep -Eq 'Reason:[[:space:]]*(;|\[@)' || ! printf '%s\n' "$files_value" | grep -Eq '^`[^`[:space:]]+`([[:space:]]*,[[:space:]]*`[^`[:space:]]+`)*$' || grep -Eq '(^/|(^|/)[.][.](/|$)|[*?[])' "$tmp_dir/exception-files" || [ "$exception_file_count" -ne "$(wc -l <"$tmp_dir/exception-files" | tr -d ' ')" ] || { [ -s "$tmp_dir/task-files" ] && ! cmp -s "$tmp_dir/task-files" "$tmp_dir/exception-files"; }; then add_error TASK_EXCEPTION "$task_relative" 'exception record is incomplete or uses a disallowed identifier'; fi
            fi
