...
- 區塊 A：`標記預覽（上方）`
  ...
  - 指定為 `evidence` 角色的欄位不在標記預覽中顯示獨立區塊；Evidence 欄位僅記錄於 config 的 `evidence_fields`，其內容留待標記工作區呈現
...
- **FR-003g-2**：Step 2 標記預覽區不得為 `evidence` 角色欄位顯示獨立卡片或區塊（所有輸出類型一致）；Evidence 欄位僅納入 config 的 `evidence_fields`，其內容留待標記工作區呈現。
...
| 版本 | 日期 | 變更摘要 |
|------|------|---------|
| 3.1.0 | 2026-07-03 | **行為變更**：Step 2 標記預覽區不再為 `evidence` 角色欄位顯示獨立卡片（FR-003g-2 反轉，所有輸出類型一致；Evidence 內容留待標記工作區呈現）；同步更新 FR-003g-3、FR-003g-4、介面定義與預覽互動隔離描述 |
