...
- Step 3 應提供可直接編輯的說明內容文字區塊（textarea），並可與上傳檔案並行使用。
...
- **FR-004a**：Step 3 指南格式必須支援 `GUIDELINE_FORMATS`，其中 `image` 受限於 `GUIDELINE_IMAGE_FORMATS`。
- **FR-004b**：Step 3 必須提供可直接編輯的說明內容欄位，且此欄位應可獨立於上傳檔案存在。
