...
讓使用者以 config-driven 的輸出類型快速辨識、搜尋與篩選任務；任務列表直接消費 `outputs[].type`，並以一至多個唯讀標籤呈現組合，不將任務壓縮回單一固定類型。Prototype 以 `docs/product/example-data/` 的 16 份任務作為可驗收示例資料，但系統能力不得被限制為這 16 筆、既有組合或固定任務名稱。
...
依 membership 語意套用單一輸出類型篩選時，這 16 筆示例的命中數必須為：`single_label = 5`、`multi_label = 2`、`single_dim = 1`、`multi_dim = 2`、`sequence_tagging = 1`、`entity_recognition = 3`、`relation_identification = 3`、`free_text = 2`。

原 13 筆示例 seed（T001–T013）一律以 `status = draft` 起始（供 `014-task-detail` 的 draft 設定編輯情境驗收）；非 `draft` 狀態的行為驗收（如刪除拒絕）以測試注入的合成任務涵蓋，不依賴示例基線。審核流程示範 seed（T014–T016，`review-flow-*.json`）為示範進行中審核流程而以進行中狀態起始：T014 為 `dry_run_in_progress`，T015–T016 為 `official_run_in_progress`。
...
- **SC-010**：Prototype 預設資料顯示上表 16 筆不同任務；`全部輸出類型` 不遺漏任一筆，且 8 個單一篩選結果命中數完全符合示例基線。
