...
- Tab B：`成員管理`
  - 區塊 1：`目前成員清單`
    ...
    - 任務角色顯示：以有色標籤區隔（`reviewer` 與 `annotator` 使用不同色彩），樣式對齊 task-list「標記模式」badge 規格（輕量標籤尺寸與邊框）
    - 成員狀態顯示：`啟用/停用` 需以 badge 呈現，樣式對齊 task-list「標記模式」badge 規格
...
- Tab C：`標記進度`
  ...
  - 區塊 2：`成員進度表`
    - 欄位：成員姓名、角色、已完成數、待完成數、平均速度、個人進度條、最後提交時間、品質旗標
...
| 1.4.7 | 2026-04-21 | 任務角色標籤樣式對齊 task-list「標記模式」badge 規格：調整為輕量標籤尺寸並沿用同色系邊框/背景/字色 |
| 1.4.8 | 2026-04-21 | 成員狀態顯示優化：`啟用/停用` 改為 badge 呈現，並對齊 task-list「標記模式」標籤樣式 |
