...
- Tab A：`任務概覽`（預設）
  ...
  - 區塊 2：`標記設定`（設定檔介面）
    - 顯示狀態：固定顯示 `設定檔版本`（顯示使用者上傳的 config 檔名；未上傳時為空字串）與 `標記類型`；其餘摘要欄位需依當前 `task_type` 的 schema 動態顯示（例如 `sequence_labeling` 顯示 `實體類型`、`標記格式`、`允許重疊標記`）
  ...
  - 區塊 4：`任務狀態與執行控制`
    - 顯示狀態：狀態列（`draft` / `dry_run_in_progress` / `waiting_iaa_confirmation` / `official_run_in_progress` / `completed`）、狀態 badge、資料隔離狀態與審計資訊
    - 編輯狀態：`project_leader` 可執行 `發布 Dry Run`、`發布 Official Run`；`reviewer` 顯示唯讀 disabled
  - 區塊 5：`試標資料抽樣設定`
    - 顯示狀態：抽樣方式、抽樣數值、抽樣策略、試標/正式資料配比、資料隔離狀態與隔離異動資訊
    - 編輯狀態：可調整抽樣方式、抽樣數值、重算抽樣、資料隔離開關（沿用既有抽樣驗證規則）
...
- `draft` 狀態需可調整試標抽樣（筆數或百分比），並即時顯示正式標記可用剩餘資料量。
...
- **FR-010m**：Overview「試標資料抽樣設定」中的 `抽樣方式` 控制元件必須使用 `radio`（`百分比` / `筆數`），並與 `task-new`「抽樣方式」的元件樣式與鍵盤可及性一致。
- **FR-010n**：Overview「試標資料抽樣設定」中的 `抽樣方式` 與 `抽樣數值` 必須標記為必填欄位，且在顯示模式與編輯模式皆需顯示紅色 `*`。
...
- **FR-014f**：Overview 必須以 6 區塊雙模式呈現：`基本資料`、`標記設定`、`說明文件上傳`、`任務狀態與執行控制`、`試標資料抽樣設定`、`匯出結果`；各區塊需明確定義顯示狀態與編輯狀態。
...
- **SC-017**：Overview「試標資料抽樣設定」中的 `抽樣方式` 與 `抽樣數值` 在顯示模式與編輯模式皆顯示紅色 `*`，並與其他必填欄位樣式一致。
