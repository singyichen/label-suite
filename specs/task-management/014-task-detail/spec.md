...
5. **Given** 位於 `annotation-results`，**When** 點擊匯出，**Then** 可匯出 `json` 或 `json-min`。
...
- Tab B：`標記結果`
  ...
  - 區塊 2：`樣本結果表`
    - 樣式：可展開階層式表格，結構對齊審核員標記作業介面，但移除所有操作功能
    - 父列欄位：樣本 ID、文本摘要（截斷顯示）、標記分布統計（各標籤出現次數）、完成狀態（待處理 / 草稿 / 已提交）、完成時間
    - 子列欄位（展開後顯示，每位標記員一列）：
      - 標記員姓名
      - 標記值（依 `task_type` 動態呈現）：
        - `single_sentence_classification`：標籤 badge 組
        - `single_sentence_va_scoring`：Valence 分數 + Arousal 分數
        - `sequence_labeling.subtype = ner`：實體 badge 組（類型 + 數量）
        - `sequence_labeling.subtype = aspect_list`：aspect 數量與片段摘要
        - `sentence_pairs`：分類標籤或評分值
      - 提交時間
...
- **FR-015b**：`annotation-results` tab 的樣本結果表必須為可展開兩層的階層式結構：父列顯示樣本摘要（樣本 ID、文本摘要截斷、標記分布統計、完成狀態、完成時間），展開後子列每位標記員各一列。
- **FR-015c**：標記員子列必須以 `task_type` 適配方式顯示標記值：`single_sentence_classification` 顯示標籤 badge 組；`single_sentence_va_scoring` 顯示 Valence 分數與 Arousal 分數；`sequence_labeling.subtype = ner` 顯示實體 badge 組；`sequence_labeling.subtype = aspect_list` 顯示 aspect 數量與片段摘要；`sentence_pairs` 顯示分類標籤或評分值。
