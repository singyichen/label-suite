...
- Tab A：`任務概覽`（預設）
  ...
  - 區塊 2：`標記設定`（設定檔介面）
    - 顯示狀態：`Config 版本`（顯示使用者上傳的 config 檔名；未上傳時為空字串）、標記類型、標籤清單、允許多選
...
| 版本 | 日期 | 變更摘要 |
|------|------|---------|
| 1.5.3 | 2026-04-22 | 同步 Overview 規格至最新 HTML：調整為 6 區塊（新增「試標資料抽樣設定」、`任務設定` 更名為 `標記設定`），並改為各區塊獨立 `編輯/儲存/取消`；`Config 版本` 改為顯示上傳檔名（未上傳為空） |
