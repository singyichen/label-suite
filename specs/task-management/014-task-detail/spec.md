...
| 7 | `annotator` | 嘗試進入 `/task-detail` | 阻擋存取並導回對應入口頁 |
...
2. **Given** `task_role = reviewer`，**When** 檢視 `member-management`，**Then** 不可執行任何編輯操作。
3. **Given** `task_role = reviewer`，**When** 進入 `work-log`，**Then** 僅可見自己的工時資料。
4. **Given** `task_role = annotator`，**When** 直接開啟 `/task-detail`，**Then** 系統阻擋並導回允許入口。
...
2. **Given** 所有 annotator 完成 Dry Run，**When** 系統檢查完成條件，**Then** 自動轉為 `waiting_iaa_confirmation` 並對 `project_leader` 發送提醒。
...
- **FR-006**：`reviewer` 在 `member-management` 不可執行編輯操作。
- **FR-007**：`reviewer` 的 `work-log` 僅可查看自己的資料。
- **FR-008**：任務狀態轉換必須遵守 `TASK_STATUSES` 狀態機。
- **FR-008a**：當所有 annotator 完成 Dry Run，系統必須自動轉為 `waiting_iaa_confirmation` 並建立提醒。
...
flowchart LR
    ...
    annotatorBlocked["annotator 直接進入 /task-detail"] --> dashboard["/dashboard"]
```
...
| `annotator` 直接造訪 `/task-detail` | 路由守門 | `/dashboard`（或指定允許入口） |
...
- **SC-003**：`annotator` 不能進入 `/task-detail`，會被導向允許入口。
