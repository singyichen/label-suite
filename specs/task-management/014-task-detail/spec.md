...
sequenceDiagram
    ...
    PL->>UI: 在 member-management 新增/調整/移除成員
...
| 2 | `project_leader` | 調整成員 | 可新增、調整、移除/停用任務成員（承接 task-new 初始值） |
...
2. **Given** 位於 `member-management`，**When** 從平台使用者清單加入/調整/移除成員，**Then** 成員列表更新且角色變更生效。
...
- **FR-005a**：`member-management` 必須先顯示由 `task-new` 帶入的初始成員，再允許後續調整。
