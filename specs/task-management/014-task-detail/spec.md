...
- Tab A：`任務概覽`（預設）
  ...
  - 區塊 3：`任務狀態與 Run 控制`
    ...
    - Draft Run 資料抽樣設定：
      - 抽樣方式：`筆數` 或 `百分比`
      - 抽樣輸入：可輸入「取 N 筆」或「取 N%」作為 Draft Run 資料
      ...
      - 即時摘要：顯示 `Draft Run 使用量` 與 `Official Run 預留量（剩餘資料）`
      - 規則：Official Run 預設使用「扣除 Draft Run 後的剩餘資料集」
      - 快照凍結：按下 `發布 Dry Run` 時，系統固定 `sample_snapshot_id` 與資料 id 清單，後續資料集更新不回溯改動本次 run 切分
    - 資料隔離設定：
      - 控制項：`是否啟用資料隔離（Dry Run 與 Official Run）`
      - 啟用時：Dry Run 與 Official Run 資料/結果分區儲存，Dry Run 不污染正式標記資料
...
- `overview` 的 Run 控制按鈕顯示規則固定化：
  - `draft`：顯示 `發布 Dry Run`
  - `waiting_iaa_confirmation`：顯示 `發布 Official Run`
  - 其他狀態：不顯示發布按鈕，只顯示狀態 badge 與說明文字
- `draft` 狀態需可設定 Draft Run 抽樣（筆數或百分比），並即時顯示 Official Run 可用剩餘資料量。
...
- `reviewer` 在 `overview` 需顯示 disabled run 按鈕（含 tooltip：`僅 project leader 可操作`），避免看不到入口而誤解。
...
任務需遵守固定狀態機，且支援設定 Draft Run 抽樣比例/筆數，並可選擇是否啟用 Dry Run 與 Official Run 資料隔離。

**此優先級原因**：讓團隊可控地切分試標資料，同時避免（或明確承擔）測試資料污染正式成果風險。  
**獨立測試方式**：模擬完整狀態轉換，驗證轉換條件、Draft 抽樣計算、Official 剩餘資料分配、隔離設定行為。
...
5. **Given** 任務為 `draft`，**When** 使用者設定 Draft Run 抽樣為 `N 筆` 或 `N%`，**Then** 系統需即時計算 Draft 使用資料與 Official 剩餘資料。
...
- **FR-010**：系統必須提供 Draft Run 抽樣設定，支援以「筆數」或「百分比」指定 Draft Run 使用資料量。
- **FR-010a**：當 Draft Run 抽樣設定完成後，系統必須將其餘未抽樣資料自動標記為 Official Run 預留資料集。
- **FR-010b**：系統必須提供「資料隔離」開關，預設為啟用；啟用時 Dry/Official 資料與結果不得混用。
- **FR-010c**：當使用者停用資料隔離時，系統必須顯示高風險警告、要求二次確認，並記錄審計資訊（操作者、時間、設定值）。
- **FR-010d**：Draft Run 抽樣輸入驗證規則必須明確：百分比 `1..99`；筆數 `>= 1` 且 `< dataset_total`，不符時阻擋發布。
