...
- `target_agreement_overrides = { [output_type]: number }`（使用者僅能對本任務 `outputs[]` 中已於 `OUTPUT_TYPE_IAA_REGISTRY` 登錄 `default_threshold` 的輸出類型覆寫其 IAA 目標門檻；屬 `IAA_UNCALIBRATED_TYPES` 或 `IAA_GATE_EXCLUDED_TYPES` 者不可覆寫，見 FR-010o-1；未設定的 key 回退至 `OUTPUT_TYPE_IAA_REGISTRY` 對應 `default_threshold`）（**v4.1.0 修訂**，issue #783）
...
10. **AC-1.10**（**v3.2.0 新增**，issue #742）：**Given** 任務 `outputs[]` 含 `sequence_tagging`，某樣本已提交 `spans[]` 形狀的標記結果，**When** `project_leader` 於 `annotation-results` 點擊匯出、於對話框維持預設（方案 `BIO`、單位 `character`）並確認，**Then** 匯出檔 metadata 記錄 `tagging_scheme` 為 `BIO`、`token_unit` 為 `character`，不含 `tokenizer.engine`、`tokenizer.version`、對齊模式或擴張筆數任一欄位，畫面未顯示任何擴張摘要；同一份標記結果改選 `BIOES` 再次匯出可正常完成，兩份檔案的 `tagging_scheme` 各自為 `BIO` 與 `BIOES`，且兩次匯出後任務設定未被寫入任何標註方案或詞元單位欄位（FR-020）。
11. **AC-1.11**（**v3.2.0 新增**，issue #742）：**Given** 任務 `outputs[]` 含 `sequence_tagging`，某樣本有一筆標記的邊界落在所選切詞引擎的 token 內部，**When** `project_leader` 於匯出對話框選擇單位 `word`、指定一個具備版本資訊的切詞引擎並確認，**Then** 匯出完成，檔案 metadata 含 `tokenizer.engine`、`tokenizer.version`、對齊模式與擴張筆數；畫面顯示「N 段標記因對齊被擴張」摘要，`N` 與 metadata 的擴張筆數一致，展開後逐筆顯示原始標記文字、擴張後文字與起訖 offset 差值；該樣本已儲存的 `spans[]` 起訖值未被改動，且同一任務改選單位 `character` 匯出時該摘要不出現（FR-020）。
12. **AC-1.12**（**v3.2.0 新增**，issue #742）：**Given** `project_leader` 於匯出對話框選擇單位 `word`，**When** 所選切詞引擎未提供版本資訊、使用者觸發匯出，**Then** 該次匯出被阻擋，畫面顯示可理解的原因並指出缺少切詞引擎版本資訊，未產生任何匯出檔、匯出記錄表未新增紀錄；使用者於同一對話框改回單位 `character` 後匯出正常完成，該次匯出的檔案不含 tokenizer metadata，畫面亦未顯示擴張摘要（FR-020）。
13. **AC-1.13**（**v3.2.0 新增**，issue #742）：**Given** 一個 `entity_recognition` 任務，其標記值為 `entities[]`，**When** `project_leader` 於 `annotation-results` 匯出該任務的結果，**Then** 匯出流程不出現標註方案或詞元單位的選擇、亦不呼叫序列推導，匯出結果仍逐欄位包含 `entities[]`（每個 entity 保留 `text`、`label` 與 span/offset 語意），`JSON-MIN` 的扁平化欄位仍為 `entities_summary`（FR-020 第 (6) 項、FR-015i-3）。
14. **AC-1.14**（**v3.3.0 新增**，issue #772）：**Given** `project_leader` 於 `sequence_tagging` 任務選定一組頁面篩選、於匯出對話框選擇標註方案 `BIOES`、詞元單位 `word` 與一個具版本資訊的切詞引擎並完成一次 `JSON` 匯出，**When** 隨後改變頁面篩選、於匯出對話框改選其他方案與單位後取消，再按下該筆匯出記錄列的「下載」，**Then** 下載的檔案內容與第一次匯出的檔案逐字元相同、檔名亦相同，檔案 metadata 的標註方案、詞元單位與切詞引擎仍為第一次匯出時的值；匯出記錄表的列數與按下「下載」之前相同，匯出對話框未被開啟，畫面未出現對齊擴張摘要，頁面篩選維持使用者改變後的值（FR-021）。
15. **AC-1.15**（**v3.3.0 新增**，issue #772）：**Given** `project_leader` 於一個非 `sequence_tagging` 任務套用審核員與審核狀態篩選後完成一次 `JSON-MIN` 匯出，**When** 隨後清除全部篩選、切換介面語言，再按下該筆匯出記錄列的「下載」，**Then** 下載的檔案內容與原始匯出逐字元相同、檔名亦相同，且匯出記錄表的列數不變（FR-021）。
16. **AC-1.16**（**v3.3.0 新增**，issue #772）：**Given** 匯出記錄表中存在一筆沒有條件快照的既有紀錄，**When** 使用者檢視該列，**Then** 該列的「下載」為停用狀態並附中文說明，點擊不產生任何檔案；**Given** 一筆 `word` 單位的 `sequence_tagging` 匯出紀錄所記錄的切詞引擎於重新下載當下已不可用，**When** 使用者按下該列的「下載」，**Then** 畫面顯示指出該切詞引擎不可用的中文原因，沒有任何檔案被產生，且匯出記錄表的列數不變（FR-021）。
...
- 抽樣進階輸入需即時驗證：`target_agreement_overrides` 中任一填寫值範圍需為 `0..1`、`min_annotators >= 2`；不符時阻擋儲存並顯示可修正錯誤訊息。`target_agreement_overrides` 含屬 `IAA_UNCALIBRATED_TYPES` 或不在本任務 `outputs[]` 中的 key 時，拒絕整筆儲存並逐項指出不允許的 key（FR-010o-1）（**v4.1.0 修訂**，issue #783）。
- IAA 計算方式（指標）一律由 `OUTPUT_TYPE_IAA_REGISTRY` 依 `outputs[].type` 自動選定並唯讀顯示，使用者不可選擇或切換計算方式；使用者僅能於 `target_agreement_overrides` 覆寫個別已校準輸出類型的目標門檻（可覆寫範圍見 FR-010o-1（**v4.1.0 修訂**，issue #783）），未覆寫時顯示 registry 預設門檻。
...
17. **AC-3.17**（**v4.1.0 新增**，issue #783）：**Given** 任務處於 `waiting_iaa_confirmation`，最新試標回合 `iaa_computation_status = pending`，**When** `project_leader` 檢視 Overview「任務狀態與執行控制」，**Then** 畫面顯示「IAA 計算中」，IAA 項不顯示任何數值、不顯示未達標警示或「未通過」判定；`開始正式標記` 為停用狀態，按鈕旁可見說明計算中的原因文字，點擊後任務狀態維持 `waiting_iaa_confirmation`（FR-010o-4）。
18. **AC-3.18**（**v4.1.0 新增**，issue #783）：**Given** 任務處於 `waiting_iaa_confirmation`，最新試標回合 `iaa_computation_status = failed`，**When** `project_leader` 檢視 Overview「任務狀態與執行控制」並點擊「重試計算」，**Then** 點擊前畫面顯示計算失敗狀態與「重試計算」操作，且不以未達標樣式呈現；點擊後該回合回到 `pending`、畫面改為「IAA 計算中」，任務狀態維持 `waiting_iaa_confirmation`，試標回合數不變（FR-010o-4）。
19. **AC-3.19**（**v4.1.0 新增**，issue #783）：**Given** 任務處於 `waiting_iaa_confirmation`，最新試標回合某輸出類型因有效標記員數 `< 2` 而為「無法計算」（`De = 0`），其餘需計算的輸出類型皆已得到數值，**When** `project_leader` 檢視 Overview「任務狀態與執行控制」，**Then** 該回合 `iaa_computation_status = done`，畫面不顯示「IAA 計算中」或計算失敗狀態，`開始正式標記` 與 `新增試標回合 R{trial_round+1}` 皆可點擊（FR-010o-4）。
20. **AC-3.20**（**v4.1.0 新增**，issue #783）：**Given** 任務處於 `waiting_iaa_confirmation` 且已完成 R1，最新試標回合 `iaa_computation_status` 為 `pending` 或 `failed`，**When** `project_leader` 檢視 Overview「任務狀態與執行控制」，**Then** `新增試標回合 R2` 為停用狀態，按鈕旁可見說明計算中或計算失敗的原因文字；點擊該按鈕不建立任何回合，任務狀態維持 `waiting_iaa_confirmation`（FR-010o-4、FR-013）。
21. **AC-3.21**（**v4.2.0 新增**，issue #868）：**Given** PL 在審核設定勾選兩位 reviewer，並把兩人都勾為 arbiter，**When** 儲存，**Then** 整筆儲存被阻擋、既有設定不被覆寫，且畫面以 `role="alert"` 與 `aria-live="assertive"` 明確提示至少保留一位未被指定為仲裁者的審核員（FR-010s-1）。
22. **AC-3.22**（**v4.2.0 新增**，issue #868）：**Given** `reviewer_ids = [W, C]` 且 `arbiter_ids = [C]`，**When** PL 儲存審核設定，**Then** 儲存成功，W 為新審核工作的有效分派對象，C 保留處理仲裁（FR-010s-1；015 FR-093）。
23. **AC-3.23**（**v4.2.0 新增**，issue #868）：**Given** `reviewer_ids` 至少一人且 `arbiter_ids = []`，**When** PL 儲存審核設定，**Then** 儲存成功，摘要與發布確認仍沿用既有未指定仲裁者警示，不新增儲存阻擋（FR-010s-1、FR-010s-2、FR-010t）。
24. **AC-3.24**（**v4.2.0 新增**，issue #868）：**Given** 已儲存 `reviewer_ids = [W, C]`、`arbiter_ids = [C]`，且成員異動使 W 被停用、只剩仲裁者 C 為啟用中 reviewer，**When** PL 嘗試發布試標或正式標記，**Then** 系統阻擋發布、顯示至少需一位未被指定為仲裁者的啟用中審核員，且不得建立回合或改變任務狀態（FR-010t）。
...
- **FR-010o-1**（**v4.1.0 修訂**，對應 AC-1.17、AC-1.18、AC-1.19，issue #783）：Overview「抽樣設定」不得提供 IAA 計算方式的可選下拉選單；每個 `outputs[].type` 的計算方式必須由 `OUTPUT_TYPE_IAA_REGISTRY` 自動選定並唯讀顯示。
  - **(1) 可覆寫範圍**：使用者只能對同時符合下列兩個條件的輸出類型於 `target_agreement_overrides` 輸入覆寫門檻：該型別在本任務的 `outputs[]` 中，且 `OUTPUT_TYPE_IAA_REGISTRY` 為其登錄了 `default_threshold`。未覆寫時必須顯示 registry 的 `default_threshold` 作為 placeholder 建議值。
  - **(2) 未校準型別不提供覆寫**：屬 `IAA_UNCALIBRATED_TYPES`（定義與成員以 `dataset/017-dataset-analysis-detail` FR-043 為唯一來源）或 `IAA_GATE_EXCLUDED_TYPES` 的輸出類型，「抽樣設定」編輯狀態不得渲染覆寫輸入框，唯讀摘要不得顯示任何門檻數值。
  - **(3) 儲存時拒絕**：儲存 `target_agreement_overrides` 時，若含有屬 `IAA_UNCALIBRATED_TYPES` 的 key、或含有不在本任務 `outputs[]` 中的 key，系統必須拒絕整筆儲存並逐項指出不允許的 key，不得靜默丟棄該 key 後儲存其餘欄位。持久化後的 `target_agreement_overrides` 不得含有屬 `IAA_UNCALIBRATED_TYPES` 的 key。
  - **(4) 校準後自動開放**：某型別自 `IAA_UNCALIBRATED_TYPES` 移出、且 `OUTPUT_TYPE_IAA_REGISTRY` 為其登錄 `default_threshold` 後，該型別的覆寫輸入框必須依第 (1) 點自動出現，不得需要修改本頁面依型別分流的程式（憲法：Generalization-First）。
...
- **FR-010o-4**（**v4.1.0 新增**，對應 AC-3.17、AC-3.18、AC-3.19、AC-3.20，issue #783）：IAA 為非同步計算。任務依 FR-008a 進入 `waiting_iaa_confirmation` 時，最新試標回合的 IAA 可能尚未算完或計算失敗；本條規定這兩種情況在畫面上的呈現，以及它們與「IAA 未達標」的區別。轉換前置條件本身以 `docs/adr/022-task-state-machine-location.md` Transition Table 為準。
  - **(1) 計算狀態欄位**：`TrialRound` 必須具備 `iaa_computation_status`，值域恰為 `pending | done | failed`：回合建立時為 `pending`；該回合每一個需計算 IAA 的輸出類型都得到確定結果時為 `done`；計算執行錯誤時為 `failed`。「確定結果」指一個數值，或 `dataset/017-dataset-analysis-detail` FR-039 第 4 點定義的「無法計算」（`De = 0`）；後者必須記為 `done`（與該規格 AC-3.16「不阻擋使用者進入正式標記」一致）。`IAA_GATE_EXCLUDED_TYPES` 不需計算，不列入判斷。本欄位只描述計算是否結束，不得承載達標與否。
  - **(2) 計算中**：最新回合為 `pending` 時，Overview「任務狀態與執行控制」必須顯示「IAA 計算中」狀態；達標條件 pills 的 IAA 項、判定 banner 與試標回合歷程不得顯示任何 IAA 數值或達標／未達標判定。
  - **(3) 計算失敗與重試**：最新回合為 `failed` 時，同一區塊必須顯示計算失敗狀態並提供「重試計算」操作；該操作只提供給 `project_leader`。重試後該回合必須回到 `pending` 並重新排入計算；重試不得改變任務狀態、不得建立新回合。
...
- **SC-048**（**v4.2.0 新增**，issue #868）：以所有 reviewer 同時為 arbiter 的設定儲存時，成功寫入次數為 0，且中英文錯誤皆以 assertive live alert 呈現；只要保留至少一位非 arbiter reviewer 或 `arbiter_ids = []`，儲存成功率為 100%，且 helper text 100% 揭露仲裁者不接收新審核單位。設定儲存後若成員異動使啟用中的 `reviewer_ids - arbiter_ids` 歸零，試標與正式發布的成功次數皆為 0，任務狀態與回合數維持不變（AC-3.21～AC-3.24、FR-010s-1、FR-010t）。
...
| 4.1.0 | 2026-09-18 | **IAA 計算狀態前置條件與門檻覆寫範圍（issue #783 第 1、2 點，OpenSpec change `task-detail-iaa-precondition-and-override-scope`，MINOR）**：**修訂**：FR-010o-1 原地改寫為 (1)–(4) 四點——`target_agreement_overrides` 只能覆寫本任務 `outputs[]` 中已於 `OUTPUT_TYPE_IAA_REGISTRY` 登錄 `default_threshold` 的輸出類型；屬 `IAA_UNCALIBRATED_TYPES`（以 `dataset/017-dataset-analysis-detail` FR-043 為唯一來源）或 `IAA_GATE_EXCLUDED_TYPES` 者不渲染輸入框、不顯示門檻數值；儲存含未校準或不在 `outputs[]` 中的 key 時拒絕整筆儲存並逐項指出；型別完成校準後輸入框自動出現。規格常數 `target_agreement_overrides` 說明、使用者故事 1 抽樣設定編輯狀態與兩條行為規則、FR-010q 與 `TaskDetail` 實體同步收斂至同一範圍。**新增**：FR-010o-4（待 IAA 確認頁顯示試標回合 IAA 計算狀態：`pending` 顯示「IAA 計算中」、`failed` 顯示失敗並提供僅限 `project_leader` 的「重試計算」、計算未結束不得呈現為 IAA 未達標、未結束時 `開始正式標記` 與 `新增試標回合 R{trial_round+1}` 皆停用並附原因；「無法計算」記為 `done`，與 `dataset/017-dataset-analysis-detail` FR-039 第 4 點、AC-3.16 一致）；`TrialRound.iaa_computation_status`（`pending | done | failed`）；AC-1.17、AC-1.18、AC-1.19（使用者故事 1）與 AC-3.17、AC-3.18、AC-3.19、AC-3.20（使用者故事 3）。**未變更**：FR-010o-3 顧問性警示原則不變（計算未結束之停用依據為「計算尚未結束」，不屬「因 IAA 未達標停用」）；FR-008a 試標完成條件不變；轉換前置條件寫在 `docs/adr/022-task-state-machine-location.md`（本版同步新增 Amendment 並更新 Transition Table），本規格不另定義狀態機。詳見 `openspec/changes/archive/2026-09-18-task-detail-iaa-precondition-and-override-scope/design.md`。 |
