...
- `user` 使用者在不同任務中同時擔任 `project_leader` 與 `annotator` 任務角色時？→ `UserDashboard` 依各任務的 `task_role` 動態渲染各自的內容區塊，不合併顯示。
...
| `UserDashboard` | 「快速繼續」按鈕 / 任務卡 | `/annotation-workspace` |
| `UserDashboard` | 「查看個人工時紀錄」（空狀態）| `/work-log` |
| `UserDashboard` | 「編輯個人資料」（空狀態）| `/profile` |
