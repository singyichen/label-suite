...
### User Story 2 — User 以 annotator 角色檢視已指派任務（優先級：P1）
...
### User Story 4 — User 以 project_leader 角色檢視任務進度（優先級：P2）

擁有 `user` 系統角色且在至少一項任務中持有 `project_leader` 任務角色的使用者，登入後可在儀表板看到其負責任務的總覽卡、待處理事項提醒（IAA 待確認 / Dry Run 全員完成），以及標記員進度區；若尚未建立任何任務則顯示空狀態引導。
...
4. **Given** 尚未建立任何任務的 project_leader（無 task_membership 的 PL 任務），**When** 導向 `/dashboard`，**Then** 顯示空狀態說明文字與「建立第一個任務」按鈕（→ `/task-new`）。
...
- **FR-013**：`UserDashboard` 中 `task_role = project_leader` 的任務區塊必須顯示：任務總覽卡（任務名稱、任務類型、當前狀態、整體完成率進度條）、待處理事項區（IAA 結果待確認附快速連結至 `/dataset-quality`；Dry Run 已全員完成待啟動 Official Run）、標記員進度區（各標記員本任務完成數 / 今日完成數 / 平均速度，速度異常者標示警示）；無任務時顯示說明文字與「建立第一個任務」按鈕（→ `/task-new`）。
...
#### US2 — User 以 annotator 角色檢視已指派任務
...
#### US4 — User 以 project_leader 角色檢視任務進度

```mermaid
flowchart TD
    A(["/login 登入成功"]) --> B["/dashboard — role = user"]
    B --> C["UserDashboard — Skeleton\nGET /users/me/tasks 載入中"]
    C -->|"回傳含 task_role=project_leader 且有任務"| D["UserDashboard — PL Block\n任務總覽卡（名稱/類型/狀態/完成率）\n待處理事項區（IAA 待確認 badge / Dry Run 全員完成提醒）\n標記員進度區（完成數/今日數/速度/異常警示）"]
    C -->|"回傳 project_leader 但尚未建立任何任務"| E["UserDashboard — PL 空狀態\n『尚未建立任何任務』\n[建立第一個任務] 按鈕"]
    D -->|"點擊 IAA 待確認快速連結"| F(["/dataset-quality（對應任務）"])
    D -->|"點擊任務卡"| G(["/task-detail（對應任務）"])
    D -->|"點擊待處理事項：啟動 Official Run"| G
    E -->|"點擊『建立第一個任務』"| H(["/task-new"])
```
...
| `UserDashboard — PL 空狀態` | ✅ | 空狀態說明文字 + 「建立第一個任務」按鈕 |
...
| `UserDashboard — PL 空狀態` | 「建立第一個任務」按鈕 | `/task-new` |
