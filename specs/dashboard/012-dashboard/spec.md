...
**獨立測試方式**：以無任務關係的 `user` 登入，確認顯示一般使用者版面與 4 個指標卡。

**驗收情境**：

1. **Given** `role = user` 且無任何任務關係，**When** 進入 `/dashboard`，**Then** 顯示「歡迎使用 Label Suite」與「建立第一個任務」按鈕。
2. **Given** 同上，**When** 檢視指標卡，**Then** 顯示：目前角色、我建立的任務、我被指派的任務、我被指派的審核。
3. **Given** 同上，**When** 檢視下一步引導，**Then** 顯示成為專案負責人 / 標記員 / 審核員的轉換說明。
...
- 引導卡 A：
  - 標題：`建立任務後成為專案負責人`
  - 內文：`建立任務後，你會在該任務中擁有管理與配置權限。`
- 引導卡 B：
  - 標題：`被指派標記工作後成為標記員`
  - 內文：`被加入標記成員後，你會看到待標記工作與快速繼續入口。`
- 引導卡 C：
  - 標題：`被指派審核工作後成為審核員`
  - 內文：`被加入審核成員後，你會看到待審任務與快速審查入口。`

**引導行為規則**：

- 當使用者符合一般使用者條件（無任務關係）時，三張引導卡必須同時顯示。
- 語言切換時，引導區塊標題/副標與三張卡的標題/內文必須即時切換。
- 「建立第一個任務」按鈕為此區塊主 CTA，預期導向任務建立流程（實際路由可於整合階段綁定）。
...
3. **Given** `role = super_admin`，**When** 檢視任務列表，**Then** 每列包含名稱、摘要、Task Type badge、Run Type badge、狀態 badge、進度條。
...
- 區塊 D：`任務列表`
  ...
  - 任務列項欄位：
    ...
    - badge 群組（Task Type、Run Type、Status）
    - 完成率 progress bar
...
2. **Given** 同上，**When** 檢視任務列表，**Then** 每列包含任務名稱、摘要、Task Type badge、Run Type badge、狀態 badge、進度條。
...
**此優先級原因**：Annotator 的主流程是快速回到標註工作。

**獨立測試方式**：以有 `annotator` 任務關係的 `user` 登入，確認顯示 Annotator 版面。

**驗收情境**：

1. **Given** `role = user` 且有 `annotator` 任務，**When** 進入 `/dashboard`，**Then** 顯示我的概況（待標記、今日完成、平均速度）。
...
1. **Given** `role = user` 且有 `reviewer` 任務，**When** 進入 `/dashboard`，**Then** 顯示待審概況（待審總數、今日已審、IAA 摘要）。
2. **Given** 同上，**When** 檢視任務列表，**Then** 每列包含名稱、審查摘要、badge、進度條與「快速審查」按鈕。
...
- **FR-007**：一般使用者 Dashboard 必須包含 4 項指標，且由左至右順序為：目前角色、我建立的任務、我被指派的任務、我被指派的審核。
- **FR-007A**：一般使用者 Dashboard 必須包含引導區塊標題與副標：`你接下來可以做什麼`、`角色會依任務行為自動轉換`。
- **FR-007B**：一般使用者 Dashboard 必須包含 3 張引導卡（專案負責人 / 標記員 / 審核員），且每張卡均包含標題與說明文字。
- **FR-007C**：一般使用者狀態下必須顯示「建立第一個任務」主 CTA；語言切換時文字需同步更新。
...
- **FR-010**：Annotator Dashboard 必須包含：我的概況、任務列表、快速繼續按鈕。
- **FR-010A**：Annotator Dashboard 的「我的概況」必須包含 3 張指標卡：待標記、今日完成、平均速度。
- **FR-010B**：Annotator Dashboard 的任務列表每列必須包含進度摘要、`快速繼續` 按鈕、3 種 badge 與 progress bar。
- **FR-011**：Reviewer Dashboard 必須包含：待審概況、任務列表、快速審查按鈕。
- **FR-011A**：Reviewer Dashboard 的「待審概況」必須包含 3 張指標卡：待審總數、今日已審、IAA 摘要。
- **FR-011B**：Reviewer Dashboard 的任務列表每列必須包含審查摘要、`快速審查` 按鈕、3 種 badge 與 progress bar。
- **FR-012**：所有任務列表列項都必須包含 badge（Task Type / Run Type / 狀態）與 progress bar。
...
- **FR-017E**：在 `<= MOBILE_BP` 時，頂部品牌列必須顯示當前人員名稱與 i18n 語言切換按鈕（例如 `ZH | EN`）；語言切換按鈕需可即時切換語系。
...
- **LanguageState**：當前語言狀態。關鍵欄位：`lang`（`zh` / `en`）。
...
- **SC-003**：一般使用者 Dashboard 顯示「我被指派的審核」指標。
- **SC-004**：一般使用者 Dashboard 的引導區塊完整顯示 3 張引導卡，且文字內容符合本規格定義。
...
- **SC-009**：Annotator Dashboard 完整顯示我的概況與任務列表，且每列皆有「快速繼續」按鈕。
- **SC-010**：Reviewer Dashboard 完整顯示待審概況與任務列表，且每列皆有「快速審查」按鈕。
