...
- `role` 為 JWT 中未定義的未知值時？→ 視同 `null`，導向 `/pending`（deny-by-default）。
...
- **FR-001**：`DashboardPage` 必須以 `role ===` 明確比對進行 role dispatch：`super_admin` → `SuperAdminDashboard`；`annotator` → `AnnotatorDashboard`；`null` 或未知值 → 導向 `/pending`。
...
- **SC-003**：`role = null` 或未知 role 的使用者導向 `/dashboard` 時，在 500ms 內被 RoleGuard 導向 `/pending`，不渲染任何 dashboard 內容。
