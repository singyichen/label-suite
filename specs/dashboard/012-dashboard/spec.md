...
sequenceDiagram
    ...
    alt role 未知 / 無效
        ...
        瀏覽器->>後端API: GET /admin/stats（Dashboard payload：角色統計、任務摘要、待處理項）
        後端API-->>瀏覽器: 平台儀表板聚合資料
    else role = user
...
Super Admin 登入後看到平台總覽儀表板：平台使用者各角色帳號數量、快速進入 `user-management` 的連結、所有任務列表（名稱、類型、狀態、完成率）以及需關注的任務待處理項。若平台剛部署尚無資料，顯示空狀態說明與「管理使用者」快捷按鈕。
...
**獨立測試方式**：以持有 project_leader task_role 的 `user` 帳號登入，確認 UserDashboard 正確渲染任務總覽卡、待處理事項區與標記員進度區；確認點擊 IAA 待確認連結可導向 `/dataset-quality/:taskId`。
...
3. **Given** 任務 A 有 IAA 結果待確認的 project_leader，**When** 點擊 IAA 待確認快速連結，**Then** 導向 `/dataset-quality/:taskId`（任務 A）。
...
**獨立測試方式**：以持有 reviewer task_role 的 `user` 帳號登入，確認 UserDashboard 正確渲染待審查任務列表與 IAA 摘要；確認點擊任務卡可導向 `/annotation-workspace/:taskId` 審查模式；確認無任務時顯示空狀態。

**驗收情境**：

1. **Given** 在任務 A 持有 `reviewer` 任務角色的使用者，**When** 導向 `/dashboard`，**Then** 顯示任務 A 的待審查任務列表（任務名稱、待審核筆數、已審核 / 總筆數）。
2. **Given** 上述 reviewer，**When** 點選任務 A 的任務卡，**Then** 直接進入 `/annotation-workspace/:taskId` 審查模式（任務 A）。
3. **Given** 任務 A Dry Run 進行中的 reviewer，**When** 檢視儀表板，**Then** Dry Run IAA 摘要顯示當前 IAA 分數與達標 / 未達標狀態。
4. **Given** 目前無待審查任務但仍具至少一個 reviewer 任務成員資格的 reviewer，**When** 導向 `/dashboard`，**Then** 顯示空狀態說明文字與「查看最近任務統計報告」次要按鈕（→ `/dataset-stats/:taskId`，其中 `taskId` 為最近有 reviewer activity 的任務）。
...
- `reviewer` 使用者目前無待審查任務，但仍有歷史 reviewer 任務時？→ 顯示 Reviewer 空狀態，並將「查看最近任務統計報告」導向最近有 reviewer activity 的任務 `/dataset-stats/:taskId`；若完全沒有任何 reviewer 任務，則不渲染 Reviewer Block。
...
- **FR-013**：`UserDashboard` 中 `task_role = project_leader` 的任務區塊必須顯示：任務總覽卡（任務名稱、任務類型、當前狀態、整體完成率進度條）、待處理事項區（IAA 結果待確認附快速連結至 `/dataset-quality/:taskId`；Dry Run 已全員完成待啟動 Official Run）、標記員進度區（各標記員本任務完成數 / 今日完成數 / 平均速度，速度異常者依 `task.config.dashboard.velocity_alert_threshold` 規則標示警示）。
- **FR-014**：`UserDashboard` 中 `task_role = reviewer` 的任務區塊必須顯示：待審查任務列表（任務名稱、待審核筆數、已審核 / 總筆數；點選任務卡 → `/annotation-workspace/:taskId` 審查模式）、Dry Run IAA 摘要（IAA 分數依任務類型顯示對應指標、達標 / 未達標狀態）、快速進入審查按鈕（→ `/annotation-workspace/:taskId`）；無待審查任務但仍具 reviewer 任務成員資格時顯示說明文字與「查看最近任務統計報告」次要按鈕（→ `/dataset-stats/:taskId`）。

### API Contracts

- **API-001**：`GET /admin/stats` 必須回傳 Super Admin Dashboard 所需的完整聚合 payload，至少包含：`role_counts`（各 system role 帳號數）、`task_summaries[]`（`task_id`、`task_name`、`task_type`、`task_status`、`completion_rate`）、`pending_actions[]`（如 `waiting_iaa` 任務快速連結所需 `task_id`）。
- **API-002**：`GET /users/me/tasks` 僅回傳使用者的 `task_memberships[]`（至少含 `task_id`、`task_role`），不得承載 Dashboard 專用聚合摘要。
- **API-003**：`GET /dashboard/user-summary` 必須回傳 User Dashboard 所需的聚合 payload，至少包含：`project_leader_blocks[]`、`reviewer_blocks[]`、`annotator_blocks[]`，以及所有快捷操作所需的 `task_id`、`run_type`、`review_mode` 等導頁資訊。
...
- **API-005**：`GET /dashboard/user-summary` 中 reviewer 空狀態使用的 `recent_reviewer_task_id`，必須定義為「最近有 reviewer activity 的任務」；排序依 reviewer 最近一次審查活動時間遞減。
...
flowchart TD
    ...
    D -->|"點擊任務卡"| H(["/task-detail/{task_id}"])
    D -->|"點擊 IAA 待確認連結"| I(["/dataset-quality/:taskId"])
    E -->|"點擊『管理使用者』"| G
```
...
| `SuperAdminDashboard — 有資料` | 點擊任務卡 | `/task-detail/{task_id}`（對應任務）|
| `SuperAdminDashboard — 有資料` | IAA 待確認快速連結 | `/dataset-quality/:taskId`（對應任務）|
...
flowchart TD
    ...
    D -->|"點擊 IAA 待確認快速連結"| F(["/dataset-quality/:taskId（對應任務）"])
    D -->|"點擊任務卡"| G(["/task-detail/{task_id}（對應任務）"])
...
| From | Trigger | To |
|------|---------|-----|
| `UserDashboard — PL Block` | 點擊任務卡 | `/task-detail/{task_id}`（對應任務）|
| `UserDashboard — PL Block` | IAA 待確認快速連結 | `/dataset-quality/:taskId`（對應任務）|
| `UserDashboard — PL Block` | 待處理事項：啟動 Official Run | `/task-detail/{task_id}`（對應任務，Official Run tab）|
...
flowchart TD
    ...
    D -->|"點擊任務卡"| F(["/annotation-workspace/:taskId\n（審查模式，對應任務）"])
    D -->|"點擊『快速進入審查』"| G(["/annotation-workspace/:taskId\n（審查模式，上次未完成任務）"])
    E -->|"點擊『查看最近任務統計報告』"| H(["/dataset-stats/:taskId"])
```
...
| From | Trigger | To |
|------|---------|-----|
| `UserDashboard — Reviewer Block` | 點擊任務卡 | `/annotation-workspace/:taskId`（審查模式，對應任務）|
| `UserDashboard — Reviewer Block` | 「快速進入審查」按鈕 | `/annotation-workspace/:taskId`（審查模式，上次未完成任務）|
| `UserDashboard — Reviewer 空狀態` | 「查看最近任務統計報告」次要按鈕 | `/dataset-stats/:taskId` |
...
| ID | 畫面狀態 | Page 名稱 | 對應 US | 需繪製內容 | 導出導航 |
|----|---------|----------|---------|-----------|---------|
| U-2 | **PL Block — 有任務** | `U-2 PL Block 有任務` | US3 | ① **任務總覽卡**：任務名稱、任務類型、當前狀態（badge）、整體完成率進度條 ② **待處理事項區**：IAA 結果待確認（badge + 快速連結 → `/dataset-quality/:taskId`）、Dry Run 已全員完成提醒（→ 啟動 Official Run）③ **標記員進度區**：各標記員本任務完成數 / 今日完成數 / 平均速度，速度異常者依 `task.config.dashboard.velocity_alert_threshold` 顯示警示圖示或醒目顏色標示 | IAA 待確認 → `/dataset-quality/:taskId`；任務卡 → `/task-detail/{task_id}`；啟動 Official Run → `/task-detail/{task_id}`（Official Run tab）|
...
| ID | 畫面狀態 | Page 名稱 | 對應 US | 需繪製內容 | 導出導航 |
|----|---------|----------|---------|-----------|---------|
| U-5 | **Reviewer Block — 有任務** | `U-5 Reviewer Block 有任務` | US5 | ① **待審查任務列表**：任務名稱、待審核筆數、已審核 / 總筆數，點擊任務卡 → `/annotation-workspace/:taskId`（審查模式）② **Dry Run IAA 摘要**：當前 IAA 分數（依任務類型顯示對應指標）+ 達標 / 未達標狀態（圖示或顏色）③ **「快速進入審查」按鈕**（主要按鈕，導向上次未完成任務的審查模式）| 任務卡點擊 → `/annotation-workspace/:taskId`（審查模式）；「快速進入審查」→ `/annotation-workspace/:taskId`（審查模式，上次未完成）|
| U-6 | **Reviewer 空狀態** | `U-6 Reviewer 空狀態` | US5 | `reviewer` task_role 存在但目前無待審查任務：說明文字（無待審查任務）+ 「查看最近任務統計報告」次要按鈕（→ `/dataset-stats/:taskId`，其中 `taskId` 為最近有 reviewer activity 的任務）；不顯示任務列表或 IAA 摘要 | 「查看最近任務統計報告」→ `/dataset-stats/:taskId` |
...
- **SC-012**：Reviewer 空狀態的「查看最近任務統計報告」導向最近有 reviewer activity 的任務 `/dataset-stats/:taskId`，排序規則與 `ReviewerActivity.reviewed_at` 一致。
