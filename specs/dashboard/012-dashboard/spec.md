...
- 區塊 B：`任務列表`
  ...
  - 任務列項欄位：
    - 任務名稱
    - 審查摘要（待審筆數 / 審核覆蓋率 / IAA；v2.6.0 起凡存在可推導審核單位狀態之任務列皆依 FR-020 推導並使用「審核覆蓋率」語意，無可推導狀態者維持既有種子摘要欄位）
    - badge 群組（Output Type tags、Annotation Stage、Status）
    - 操作按鈕：`快速審核`
    - 完成率 progress bar
