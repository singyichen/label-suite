...
Project Leader 或 Reviewer 進入任務分析詳情頁後，系統需先解析任務上下文並渲染共用 detail shell，包含麵包屑、任務基本資訊與雙 Tab 導覽。
...
4. **Given** 使用者為 `annotator`，**When** 嘗試進入詳情頁，**Then** 導回 `/dashboard`。

**介面定義（需與 IA 導覽語意一致）**：

- 區塊 A：`Detail Header`
  - 必要元素：麵包屑返回任務列表、任務名稱、任務類型、task_id 對應上下文資訊
- 區塊 B：`Tab Shell`
...
使用者在品質監控 tab 可查看 Dry Run 完成後依 `task_type` 選定的主要 IAA 指標、與閾值的比較結果，以及異常偵測與標記員個別分析。
...
6. **Given** Dry Run 尚未完成（`QUALITY_EMPTY_STATE_TRIGGER`），**When** 進入品質監控，**Then** 顯示「IAA 報告將在 Dry Run 完成後產生」與「前往任務詳情」次要按鈕。
...
- 統計總覽於 `STATS_EMPTY_STATE_TRIGGER` 觸發時，需區分「無標記資料」與「資料載入中」。
- 品質監控於 `QUALITY_EMPTY_STATE_TRIGGER` 觸發時，需區分「Dry Run 進行中」、「全員完成但報告未生成」與「結果運算中」。
...
- **FR-003**: 僅 `project_leader` 或 `reviewer` 任務角色可進入 detail 頁；`annotator` 需導回 `dashboard`。
...
- **StatsTabState**: stats tab 狀態，包含 `shared_metrics`、`task_type_stats`、`empty_state`、`loading_state`。
...
- **QualityTabState**: quality tab 狀態，包含 `iaa_report`、`anomaly_detection`、`annotator_profiles`、`empty_state`、`loading_state`。
