...
Project Leader 或 Reviewer 由 Navbar 進入資料集分析模組後，可看到自己具成員資格的任務列表，作為後續進入 analysis detail 的唯一入口。

**此優先級原因**：本頁是資料集分析模組的 L1 landing；若沒有此頁，使用者無法選擇分析任務。  
**獨立測試方式**：以具多個任務的 `project_leader` / `reviewer` 帳號進入頁面，驗證列表資料與導向行為正確。

**驗收情境**：

1. **Given** 使用者具 `project_leader` 或 `reviewer` 任務角色，**When** 點擊 Navbar「資料集分析」，**Then** 導向 `/dataset-analysis`，顯示該使用者具成員資格的任務列表。
...
4. **Given** `annotator` 角色使用者嘗試進入 `/dataset-analysis`，**When** 系統驗證角色，**Then** 導回 `/dashboard`。
...
- `annotator` 嘗試直接進入 `/dataset-analysis`：導回 `/dashboard`。
...
- **FR-002**: 僅 `project_leader` 或 `reviewer` 任務角色可進入頁面；`annotator` 進入時需導回 `dashboard`。
...
- **TaskSummaryCard**: 任務列表卡片資料，至少包含 `task_id`、`task_name`、`task_type`、`overall_completion_rate`、`iaa_status`。
