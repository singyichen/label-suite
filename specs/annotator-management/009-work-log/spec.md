...
- **FR-001**：`/work-log` 只有已登入使用者可存取；`annotator` 只能查看自己的紀錄，任務 `project_leader` 可查看其負責任務的成員紀錄。
- **FR-002**：系統必須在使用者進入與離開 `annotation-workspace` 時自動記錄時間，作為任務標記時間的計算依據。
- **FR-003**：頁面必須顯示各任務的累計標記時間、完成筆數（Dry Run / Official Run 分開）。
- **FR-004**：頁面必須支援依任務篩選。
- **FR-005**：PL 查看成員工時時，只能查看自己負責任務中的成員資料，不得存取非所屬任務的成員資料。
- **FR-006**：空狀態（尚無工時紀錄）顯示說明文字「尚無工時紀錄」。
