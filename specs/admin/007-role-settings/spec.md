...
- **RolePermission（角色權限）**：儲存每個角色對各頁面的存取設定。關鍵屬性：
  - `role`：角色識別（系統角色如 `annotator`、`super_admin`，或任務角色如 `project_leader`、`reviewer`、`annotator`）
...
- **FR-002**：頁面必須顯示所有角色的功能存取範圍，分為系統角色（`annotator`、`super_admin`）與任務角色（`project_leader`、`reviewer`、`annotator`）兩組。`role = null`（待指派狀態）無功能存取範圍，不列於設定項目中。
