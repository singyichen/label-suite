...
sequenceDiagram
    ...
    alt email 尚未被使用
        ...
        後端API->>資料庫: 建立使用者記錄（name、email、hashed_password、role = user）
        資料庫-->>後端API: User record
        後端API-->>瀏覽器: 201 Created + 導向 /dashboard
    else email 已存在
...
| 4a | 後端 | email 未被使用 | bcrypt 雜湊密碼，建立帳號（`role = user`），導向 `/dashboard` |
...
使用者從登入頁進入 `/register`，填寫姓名、Email、密碼，送出後系統建立帳號（`role = user`）並導向 `/dashboard`，立即可使用系統。

**此優先級原因**：自行註冊是使用者加入系統的入口之一，沒有這個功能，只能依賴 Super Admin 預先建帳號。

**獨立測試方式**：以新的 Email 填寫表單送出，驗證資料庫建立 `role = user` 的使用者記錄，且頁面導向 `/dashboard`。

**驗收情境**：

1. **Given** 未登入使用者在 `/register`，**When** 填寫有效的姓名、Email、密碼（≥ 8 字元）並送出，**Then** 建立 `role = user` 的使用者記錄並導向 `/dashboard`。
2. **Given** 使用者在 `/register`，**When** 點擊「取消」或「返回登入」，**Then** 導回 `/login`，不建立任何記錄。
3. **Given** 已登入使用者，**When** 直接訪問 `/register`，**Then** 自動導向 `/dashboard`。
...
- **FR-005**：新建帳號預設 `role = user`，註冊完成後立即取得平台功能存取權。
- **FR-006**：註冊成功後導向 `/dashboard`。
...
flowchart LR
    ...
    register -->|"註冊成功（role = user）"| dashboard
...
|------|---------|-----|
| `/login` | 點擊「前往註冊」 | `/register` |
| `/register` | 送出成功 | `/dashboard` |
| `/register` | 點擊「返回登入」 | `/login` |
| `/register` | 已登入使用者訪問 | `/dashboard`（自動導向）|

**Entry points**：`/login` → 「前往註冊」連結；未登入時可直接訪問 `/register`。
**Exit points**：成功 → `/dashboard`；取消 → `/login`。

### 關鍵實體

- **User（使用者）**：關鍵屬性：`id`、`email`、`name`、`hashed_password`（Email 註冊帳號為 bcrypt hash；純 Google SSO 帳號為 `null`）、`provider = email`、`provider_id = null`、`avatar_url = null`、`role = user`、`created_at`。

---

## 成功標準 *(必填)*

- **SC-001**：使用者可在 60 秒內完成完整註冊流程（填表 → 送出 → `/dashboard`）。
- **SC-002**：密碼絕不以明文儲存；資料庫中只存 bcrypt hash。
- **SC-003**：以重複 Email 註冊時，頁面顯示明確錯誤訊息，且不建立新帳號。
- **SC-004**：成功註冊建立唯一一筆 `role = user` 的使用者記錄。
