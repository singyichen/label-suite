...
40. **AC-3.40（v4.25.0 新增，對應 FR-070，issue #451）**：**Given** `role=reviewer` 且同一筆樣本，**When** 分別以 `run_type=dry_run` 與 `run_type=official_run` 進入工作區，**Then** 決策列上方的審核說明（`ws-review-note`）文字內容兩者完全相同（run-type-invariant，沿用 AC-3.33）；**And** 該文案不得以無條件語氣宣稱退回會使標記員狀態回到待標記，必須同時明文陳述「`official_run` 退回後該樣本回到待標記並產生標記員重標待辦」與「`dry_run` 退回不改變標記員狀態，品質問題由任務層級 IAA 閘門與下一輪試標處理」兩項後果，並將「審核決策」與「回退標記員狀態」描述為不同層級的效果；**And** en 文案表達與 zh 完全相同的三項語意（決策層級、`official_run` 重標待辦、`dry_run` 不回退）；**And** 通過/退回按鈕（`ws-review-row-approve` / `ws-review-row-reject`）在兩種 `run_type` 皆維持渲染，不得因本條移除或停用任一按鈕；**And** 實際送出效果必須與文案一致——`dry_run` 判定退回並送出後，該標記員 submission 狀態維持 `submitted`；`official_run` 判定退回並送出後回退為 `pending`（沿用 FR-014I、AC-3.15、AC-6.4）。
...
- **FR-070**（v4.25.0 新增，對應 AC-3.40，issue #451）：**審核決策說明文案必須與各 `run_type` 的真實效果一致**。FR-014P 決策列所屬審核卡內渲染的審核說明（`ws-review-note`，自 issue #399 起實際渲染於畫面）必須同時滿足下列四項：
  ...
  2. **必須區分兩個層級的效果**：「退回」作為審核決策（一律記錄決策與修正差異、進入 FR-051 狀態機推導）與「回退標記員狀態」（僅 `official_run` 觸發 FR-014I，於標記員端產生重標待辦）必須分開陳述；`dry_run` 的退回須明文說明不改變標記員狀態，個別標記品質問題由任務層級 IAA 閘門與下一輪試標處理（沿用使用者故事 3 適用範圍收斂說明）。
...
- **FR-072**（v4.27.0 新增，對應 AC-1.22，issue #450）：**審核員任務摘要必須由審核單位狀態推導**。`annotation-list` 任務資訊卡（FR-007C）與 dashboard 任務卡（012 FR-020）呈現的審核員進度摘要，不得取用任何預先組好的顯示字串，必須由審核單位狀態（FR-051 `REVIEW_UNIT_STATUS` 五態）即時推導：
  ...
  5. **顯示文字由計數組成**：顯示字串由 `formatReviewSummary()` 依計數組出（覆蓋率恆顯示；待審／未定稿／爭議僅於大於 0 時顯示；IAA 以結構化數值附加於末），中英文各一份；種子資料僅得保留結構化數值（如 IAA），不得保留預先組好的摘要字串。
  6. **無審核單位狀態時之回退**（Generalization-First）：回退判定條件為「該任務是否存在任何已儲存的審核單位狀態」（`derivable`），而非任務 ID 白名單；無任何可推導狀態的示範任務維持既有種子摘要與進度值，行為不變。
...
- **FR-077**（v4.32.0 新增，對應 AC-3.42，issue #453）：**審核送出前必須明示「直接修正」與「通過／退回」的關係與送出後果**。FR-044 使直接修正控件同時作為顯示與編輯用途（不另呈現唯讀答案列），FR-014P 的通過／退回決策則存放於另一份狀態；兩者在畫面上讀起來像同一個動作，實際互不相干，因此必須補足下列四項呈現層契約：
  ...
  3. **送出前確認區**：送出區必須逐輸出類型陳述原答案、修正後答案、當前決策與該組合的保存後果（通過未改／通過已改／退回未改／退回已改／尚未決策五種語意各自相異），並列出尚未決策的輸出類型名稱與筆數，以及送出後對標記員狀態的實際效果。
