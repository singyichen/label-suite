...
sequenceDiagram
    ...
    AN->>AL: 從 Annotator dashboard 任務列點擊「快速繼續」
    AL->>Query: 讀取 task_id / role / run_type / list filters
    AL-->>AN: 顯示可標記資料清單
    AN->>UI: 點擊單筆資料進入 workspace
    ...
    RV->>AL: 從 Reviewer dashboard 任務列點擊「快速審核」
    RV->>UI: 由清單選取待審資料進入 workspace
...
|------|------|--------|----------------|
| 1 | `annotator` / `reviewer` | 由 dashboard 或 navbar 進入 `annotation-list` | 讀取 query context（`task_id/role/run_type`）並載入清單 |
| 2 | 使用者 | 在清單點擊單筆資料 | 導向 `annotation-workspace` 並帶入 `task_id/sample_id/run_type/role` |
| 3 | 系統 | 載入工作區資料 | 套用固定樣本、說明檔案與進度（原型內建資料） |
| 4 | `annotator` | 進行逐筆標記、儲存、提交 | 更新完成數，提交後預設進入下一筆 |
...
### User Story 1 — 先進標記清單再進單筆作業（優先級：P1）

Annotator / Reviewer 進入標記模組時，必須先到 `annotation-list`，點擊單筆資料後才進入 `annotation-workspace`。

**此優先級原因**：IA 已定義標記清單為模組入口，若跳過清單會與導覽設計不一致。  
**獨立測試方式**：分別由 dashboard 與 navbar 進入標記模組，驗證都先落在清單頁，且可由單筆導向工作區。

**驗收情境**：

1. **Given** 使用者點擊 dashboard 任務卡「快速繼續/快速審核」，**When** 進入標記模組，**Then** 先顯示 `annotation-list`。
2. **Given** 使用者於清單點擊任一筆可操作資料，**When** 觸發導頁，**Then** 導向 `annotation-workspace` 並帶入 `task_id/sample_id/run_type/role`。
3. **Given** 使用者從工作區返回清單，**When** 回到 `annotation-list`，**Then** 保留當前任務上下文與捲動位置。
4. **Given** 清單中某筆資料被他人鎖定，**When** 點擊該筆，**Then** 顯示鎖定狀態並提供唯讀檢視或稍後再試。
5. **Given** 使用者於標記清單切換完成狀態篩選，**When** 選擇 `已提交/草稿/待處理`，**Then** 清單只顯示符合該完成狀態的資料列。
...
- `annotation-list` 為標記模組唯一入口，`annotation-workspace` 不作為 L1 landing。
...
- **FR-001**: 系統必須提供 `annotation-list` 作為 `annotator` 與 `reviewer` 的標記模組入口頁（L1）。
...
- **FR-008**: 點擊清單任一可操作資料時，必須導向 `annotation-workspace` 並帶入 `task_id/sample_id/run_type/role`。
...
```mermaid
flowchart LR
    D["/dashboard"] --> AL["/annotation-list?task_id=...&role=..."]
    D --> NAV["Navbar: 標記作業"]
    NAV --> AL
...
| From | Trigger | To |
|------|---------|----|
| `dashboard（annotator 視圖）` | 點擊任務列 `快速繼續` | `annotation-list?task_id=...&role=annotator` |
| `dashboard（reviewer 視圖）` | 點擊任務列 `快速審核` | `annotation-list?task_id=...&role=reviewer` |
| `annotation-list` | 點擊單筆資料 | `annotation-workspace?task_id=...&sample_id=...` |
...
**Entry points**: `dashboard` Annotator/Reviewer 任務列表（快速繼續/快速審核）、Navbar 標記作業（先進 `annotation-list`）。  
**Exit points**: `annotation-workspace` 返回 `annotation-list`、`annotation-list` 透過 Sidebar 切換至 `dashboard`、任務完成導引返回 `dashboard`。
...
- **SC-002**: Dashboard / Navbar 入口皆先進 `annotation-list`，再由單筆導向 `annotation-workspace`，導頁成功率達 100%。
