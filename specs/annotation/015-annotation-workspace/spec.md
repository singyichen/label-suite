...
22. **AC-1.22（v4.27.0 新增，對應 FR-072，issue #450）**：**Given** `role = reviewer` 且任務存在可推導的審核單位狀態（如 T015 `official_run`：4 個審核單位，含 1 筆待審、1 筆爭議、2 筆已定稿），**When** 進入 `annotation-list` 任務資訊卡與 dashboard 任務卡，**Then** 兩處摘要皆顯示 `待審 1 筆 · 審核覆蓋率 75% · 未定稿 2 筆 · 爭議 1 筆`（後接 IAA），數值與同頁審核單位列狀態一致；**And** 該待審單位於工作區送出通過後重新進入，兩處摘要皆變為 `審核覆蓋率 100% · 未定稿 1 筆 · 爭議 1 筆` 且不再出現「待審」，reload 後數值不變；**And** 覆蓋率達 100% 但仍有未定稿單位的任務（如 T016）必須同時揭露未定稿與爭議筆數，且狀態徽章不得顯示 `已完成`；**And** 無任何已儲存審核單位狀態的任務（如 T001）維持既有種子摘要文字與進度值不變。
