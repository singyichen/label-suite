...
| `ws-review-section-label`（v3.3.0 新增） | 合併審查列的型別標籤 | `dry_run` 標於各比對分區之首；`official_run` 標於標題列各組通過/退回按鈕之前（FR-014N） |
...
6. **AC-6.6（v3.3.0 新增）**：**Given** `run_type=official_run`，**When** reviewer 進入工作區，**Then** (1) 中欄不得渲染「原始文本」卡（`ws-review-source-text`），樣本文本僅出現於作答面板內一次（見 FR-014O）；(2) `outputs[]` 同時含 `entity_recognition` 與 `relation_identification` 時兩者合併為單一審查列，標題列並列兩組附型別標籤的通過/退回按鈕，列尾僅一個 `ws-review-correct-span` 修正控件（見 FR-014N）。
