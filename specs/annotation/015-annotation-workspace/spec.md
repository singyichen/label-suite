...
Reviewer 在同一工作區執行審查，先查看單一句子的 IAA 統計與各標記員結果，再以通過 / 退回完成逐列或批次審核，並追溯每筆決策歷程。
...
1. **Given** `role=reviewer`，**When** 進入工作區，**Then** 在 Dry Run 與 Official Run 都顯示 reviewer 可用操作（通過 / 退回），且先顯示句子級 IAA 統計摘要。
...
4. **Given** reviewer 在 Dry Run 審查，**When** 需要產生標準答案，**Then** 可使用 IAA 統計、各標記員結果與手動確認流程完成該筆決策。

**介面定義（需與 IA 導覽語意一致）**：

- 區塊 A：`中欄審查操作區`
  - 必要元素：句子級 IAA 統計摘要（`mean`、`std`、`±1.5std` 範圍）
  - 必要元素：標記員逐列結果（帳號、標記值）
  - 必要元素：逐列決策按鈕（`通過` / `退回`）
