...
| annotation-015 | Annotation List + Workspace | annotation | `review` | `feat/annotation/015-workspace-output-types` | spec v2.1.0；規格狀態：In Progress；workspace 與 list 全面遷移 ADR-029 `outputs[]` 8-key 模型：4 參數 URL 契約（`task_id`/`sample_id`/`role`/`run_type`）、registry-driven 標記面板（重用 task-config engine）、Reviewer 全 8 型直接修正（重用 annotator 控制項）、FR-024M Output-role prefill（013 FR-003g-5）+ Data Fairness 剝除邊界、guideline 面板 `guidelineFiles` 通用建模 |
