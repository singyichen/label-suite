...
| task-management-010 | Task List | task-management | `in-progress` | `feat/task-detail-settings-sync` | spec v2.1.0；規格狀態：In Progress；列表改由 `outputs[].type` 顯示多 tag，提供 registry-driven 8 輸出類型 membership 篩選與 `limit`／`offset` 分頁；13 份 example-data fixture 僅為 prototype 示例且 `status` 一律 `draft`（非 `draft` 情境以注入示例或 URL 覆寫驗收），不構成系統上限；012 Dashboard、014 Task Detail Overview 與 016 list 已同步，014 其他分頁及 results／export legacy consumer、015／017 仍延後 |
| task-management-013 | New Task (+ Config Builder) | task-management | `in-progress` | `feat/task-list-output-types` | spec v6.4.4；規格狀態：Draft；Step 1 選擇狀態明確為 `selected_categories[]` + `input_type` + `selectedOutputTypes[]`，一對一產生 ordered `outputs[].type`；010／012／014 Overview／016 consumer 已同步；014 其他分頁及 results／export legacy consumer、015／017 仍延後 |
| task-management-014 | Task Detail (incl. task-member-management/work-log) | task-management | `in-progress` | `feat/task-detail-settings-sync` | spec v2.0.1；規格狀態：In Progress；任務概覽的基本資料與標記設定同步 New Task Step 1／Step 2 的功能、元件視覺與互動狀態，依分類、輸入、欄位角色與 ordered `outputs[]` registry/schema config-driven 呈現；T001–T013 僅為示例，第 14 筆合法組合無需新增核心分支；其他分頁及 results／export legacy consumer、015／017 仍延後 |
...
| 日期 | 更新內容 |
|------|----------|
| 2026-07-29 | `task-management-010` 更新至 v2.1.0：13 筆 prototype 示例任務 `status` 基線改為一律 `draft`，供 Project Leader 直接從列表驗收任務詳情（基本資料／標記設定同步 013 Step 1／Step 2）的草稿編輯流程；`TASK_STATUS_ENUM` 與「僅 `draft` 可刪／可編輯」規則不變，非 `draft` 情境改以注入示例或 URL `status` 覆寫驗收；010 分支併入 `feat/task-detail-settings-sync`。 |
| 2026-07-29 | `task-management-014` 更新至 v2.0.1：依 `label-suite-design` 規範補齊 Task Detail 編輯模式與 013 Step 1／Step 2 的元件級視覺／互動同步契約，涵蓋 dataset field mapper、taxonomy chips、output accordion、annotation preview、Visual／Code 工具區與 1440／1024／768／375px 驗收；Task Detail 自身頁首、tabs 與外層 Overview card 保留。 |
| 2026-07-29 | `task-management-014` 更新至 v2.0.0 並於 `feat/task-detail-settings-sync` 進入 `in-progress`：任務概覽的基本資料與標記設定同步 `task-management-013` Step 1／Step 2 consumer 契約，以 JSON 資料集、欄位角色、taxonomy 選擇狀態與 ordered `outputs[]` registry/schema 驅動摘要、Visual／Code／預覽及輸出變更；T001–T013 僅為 prototype 示例，第 14 筆合法組合不需核心分支，隱藏 ground truth 不得下發。同步 `task-management-013` v6.4.4 與 `task-management-010` v2.0.2 的下游界線；014 其他分頁及 results／export legacy consumer、015／017 維持延後。 |
