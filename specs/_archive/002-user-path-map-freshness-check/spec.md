...
2. **AC-2.2**：**Given** `design/system/screen-inventory.md` 目前的畫面或視圖 ID 清單改變，使即時重算的指紋與 `<meta>` 記錄值不同，**When** 執行 checker，**Then** exit `1`（`PATH_MAP_STALE_FINGERPRINT`），指出需依 #645 流程重新實走並更新 `<meta>` 值，且同時印出記錄值與即時重算的 `sha256:<hex>`。
...
5. **AC-2.5**：**Given** 只有受監看 ID 清單之外的內容發生變更，包括未增減任何畫面／視圖 ID 的 `design/prototype/pages/**` 編輯，**When** 執行 checker，**Then** 不得僅因此判為 stale。
...
- **FR-004**：Stage 2 的受監看來源必須且只包含 `design/system/screen-inventory.md` 目前渲染的畫面 ID（`## 畫面 × 元件`）與視圖 ID（`## 同頁多重視圖`），除非後續 spec 變更明確修訂此集合；`design/prototype/pages/**` 不再直接監看。
- **FR-005**：Stage 2 必須只從 `design/system/user-path-map.html` `<head>` 內唯一的 `<meta name="path-map-screen-fingerprint" content="sha256:<64 碼小寫 hex>">` 取得記錄值；不得使用 git revision、mtime 或其他相容 fallback。
- **FR-006**：Stage 2 必須依 screen inventory 即時重算 sha256 指紋並與記錄值逐位元比較，相符為 fresh、不符為 stale，metadata 或 inventory 無法判斷時為 configuration failure；指紋輸入的排序與序列化語意以 2026-09-18 design amendment 為準。checker 不呼叫 `git`。
...
- **SC-003**：#645 完成後的 design amendment 明確記錄權威 metadata locator、指紋比較語意與 stable diagnostics；不得由實作 agent 自行補完。
- **SC-004**：Stage 2 committed QA Red 覆蓋 fresh、指紋不符 stale、invalid metadata／inventory 與 unmonitored-content negative control；paired Green 後全部通過，且真實 repository checker exit `0`。
