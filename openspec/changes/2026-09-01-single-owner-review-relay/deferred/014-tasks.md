...
> 移出緣由見 [README.md](README.md)。原群組編號保留,便於與 `tasks.md` 的相依描述對照;companion change 提案時重新編號。

## 原群組 5 — 014 審核設定與審核指派區塊(FR-005j/FR-005k/FR-010s/FR-010s-1/FR-010s-2/FR-010t)
...
> **相依**:本 change 群組 1(常數改版;`AR_REVIEW_STATUS` 需與 `REVIEW_UNIT_STATUS` 同步)。
...
## 原群組 6 之 014 側任務(FR-008b/FR-018)

> 原群組 6 的 015 側(6.1–6.3,例外池處置畫面)留在 `tasks.md`;以下 014 側入口/閘門任務移出。
