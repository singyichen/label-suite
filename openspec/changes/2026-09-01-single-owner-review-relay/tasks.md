...
- [ ] 7.4a 改寫 `design/prototype/tests/annotation/annotation-review-flow-demo-{seed,workspace,rows}.spec.ts` 之 T016／T017 契約為新模型：T016 = 審核員修正 → 仲裁採 B → 定稿；T017 = 仲裁 `兩者皆非` → 最終例外池 → 專案負責人收尾；移除 `已同意`／`已修改`／多審核員定稿門檻／票數表等舊模型斷言。驗證：執行三檔且失敗原因為種子資料尚未改寫 [@senior-qa]
...
- [ ] 7.4b 於 `design/prototype/pages/annotation/annotation-workspace.data.js` 之 `seedReviewFlowDemo()` 改寫 T016（`ofm-*`）與 T017（`oft-*`）種子列為 7.4a 契約所述之兩條路徑。驗證：三個 demo spec 全綠 [@senior-frontend]
