...
- [x] 2.5 修正群組 1.6 清單中因兩向決策移除而失效的既有測試斷言，並執行 `cd design/prototype && pnpm playwright test tests/annotation`。驗證：`tests/annotation` 全綠 [@senior-qa]
  ...
  - **（任務 3.3 起手對帳補正）**：上表原列 25 檔加總為 124，與同段記載的 126 failed 差 2 筆。於本附錄量測所在的 commit `d991163f` 重跑候選檔確認，缺漏的正是 `issue-398-review-correction-decision-desync` 1 與 `issue-399-review-decision-a11y` 1（皆為群組 2 三向決策轉綠後、`退回` 控件消失所致），已補入上一行；補正後清單加總 126，與實測相符。
  - **（任務 3.3 起手獨立基準線）**：群組 3 於 `origin/main` `b25478e7`（含群組 3 任務 3.1／3.2 已合併）以 `PW_PORT=8987`、`--workers=2` 重量測：`tests/annotation` **579 passed / 140 failed（共 719 tests）**。與本附錄的 126 差 14 筆，來源已逐筆查明：
    - 本附錄量測於 commit `d991163f`，早於任務 3.2 的仲裁版面 Green（`29b1e557`）合併；該 Green 使既有仲裁測試連帶失效 **+11**：`issue-454-arbitration-vote-context` 3→6、`annotation-workspace-arbitration` 1→4、`issue-308-finalized-unit-lock` 1→2、`annotation-review-flow-demo-seed` 6→7、`issue-578-reason-required` 0→2、`annotation-workspace-arbitration-concurrent-save` 0→1。其中五檔（`issue-454`／`annotation-workspace-arbitration`／`issue-308`／`issue-578-reason-required`／`annotation-workspace-arbitration-concurrent-save`）歸屬群組 3、由 3.7 收尾；`annotation-review-flow-demo-seed` 的 +1 併入其原有的群組 7 歸屬。
    - 另 **+3 為機器競用造成的 flaky**：`issue-596-review-assignment` 2、`annotation-workspace-review-identity` 1——量測當下另有一支 agent 以 `--workers=4` 同時跑全套；此二檔單獨重跑兩次皆全綠。**扣除 flaky 後 origin/main 的確定性基準線為 582 passed / 137 failed**。
