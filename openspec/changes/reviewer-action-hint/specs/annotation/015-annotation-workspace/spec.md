...
`annotation/015-annotation-workspace` 的 derived capability delta；正典為 `specs/annotation/015-annotation-workspace/spec.md` v4.55.0。本變更（issue #526）新增正典 FR-084 與 AC-4.47 ~ AC-4.50，並修訂 FR-064 第 7 點第 6 項之範圍註記。
...
- **GIVEN** T017 `oft-05-pending-review`（`pending`）、T016 `ofm-01-unanimous-gold`（`finalized`）、T015 `ofs-05-not-submitted`（`getReviewUnitStatus` 為 `null`）
...
本需求對應正典 FR-064 第 7 點第 6 項。原文將「角色相依提示（issue #526）」與 `ws-review-note` 文案並列為不屬第 7 點之範圍；本版於該項補註：角色相依提示已由 FR-084 落地為橫幅之**下一個兄弟元素**（`ws-review-action-hint`），仍不屬第 7 點之橫幅組成契約，AC-4.37 之橫幅子元素序列逐字不變。FR-064 其餘文字、第 1–8 點之全部契約、AC-4.27／AC-4.32／AC-4.35 ~ AC-4.42 皆不變。
