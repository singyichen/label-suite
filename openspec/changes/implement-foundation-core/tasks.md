...
- [x] 1.3 實作 `app/schemas/common.py`（`AppBaseModel`、`ErrorDetail`、`ErrorResponse`、泛型 `PaginatedResponse[T]`、`HealthResponse`）；驗證：`uv run pytest tests/core/test_schemas.py -q` 綠且 `uv run mypy .` exit 0 `[@senior-backend]`
...
>
> 7.3 補的是第 6 組驗收揭露的缺口：SC-019 有兩半，第 6 組只完成「檔案必須存在」那半；「不得使用 inline string array」目前僅因無人呼叫 `useQuery` 而成立，CI 沒有任何機制在第一個 feature service 寫出違規時攔下。`routes` 邊界型別同理，要等 `routes/` 真的存在才驗得了。
