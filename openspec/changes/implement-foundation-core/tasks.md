...
- [ ] 1.1 以 uv 建立 `backend/` 骨架（`pyproject.toml` 含 `requires-python`、fastapi、pydantic-settings，dev 依賴 pytest/httpx；`app/__init__.py`、`tests/`）；驗證：`uv run pytest tests/ -q` 可執行（收集 0 個測試、exit 0）且 `uv run ruff check .` exit 0 `[@senior-backend]`
