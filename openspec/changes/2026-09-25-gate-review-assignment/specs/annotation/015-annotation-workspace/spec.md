...
**本版修訂（issue #921，工作區側之指派閘門）**：前文「不由審核員自行挑單」與「每個審核單位恰有一位指派審核員」此前只由 `annotation-list`（`filterToAssignedUnits()`，issue #824）落實；`annotation-workspace` 從未檢查指派，任一在職審核員皆可在工作區左欄點選或以直接網址開啟他人審核單位並送出決策，造出本條明文禁止的多提交形狀。本版起：（1）工作區左欄、上一筆／下一筆導覽、送出（或略過）後之自動前進，MUST 只在指派給目前審核員的單位之間運作，與 `annotation-list` 的過濾邏輯同源（`getAssignedReviewUnits()`）——兩頁對「這位審核員能走訪到哪些單位」MUST NOT 給出不同答案；具仲裁資格（FR-060）之爭議單位入口不受本點限制，MUST 仍可經工作區左欄或直接網址抵達，理由與前文之 `arbiter_ids` 保留規則相同——仲裁者資格與審核指派是兩份名冊。（2）審核員以直接網址開啟未指派給自己的審核單位時，工作區 MUST 仍顯示樣本內容（標記員原答案），MUST NOT 渲染任何可送出的審核控件（含經鍵盤捷徑之送出路徑，FR-058），並 MUST 顯示「本單位未指派給你」之類的明確原因說明；此唯讀呈現與已定稿單位（FR-094）、離冊審核員之唯讀呈現同構，MUST NOT 實作為完全擋下的無權限頁——真正的存取控管屬於後端職責，非本規格範圍。（3）本點之閘門判定 MUST 晚於仲裁分支與已定稿分支：具仲裁資格者之爭議單位入口，以及已定稿單位之唯讀卡（FR-094），皆優先於本點之唯讀呈現。（4）本點之閘門 MUST 沿用既有之 `getAssignedReviewUnits()` 推導，MUST NOT 另立第二套指派判定。
...
#### Scenario: 工作區左欄只列出指派給自己的審核單位

- **GIVEN** `official_run` 任務勾選了數位審核員，系統依 FR-093 之位置性分派把單位分給不同審核員
- **WHEN** 其中一位審核員開啟工作區
- **THEN** 左欄只列出指派給該審核員的審核單位，與其在 `annotation-list` 看到的清單列數、單位集合一致
- **AND** 上一筆／下一筆導覽與送出後之自動前進只在這些單位之間移動
...
- **WHEN** C 開啟 U 的工作區（經左欄或直接網址皆可）
- **THEN** 左欄仍列出 U（或 U 仍可經直接網址開啟），且渲染仲裁卡與可送出之仲裁控件
