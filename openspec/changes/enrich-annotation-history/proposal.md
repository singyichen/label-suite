...
調研 Label Studio（annotation-history API：16 欄位、9 值 action enum）、CVAT、INCEpTION、Argilla、doccano、Prodigy 與 W3C Web Annotation Data Model 後，缺口收斂為四項：結果快照、耗時、`action` 常數化、理由欄位。此外 `action` 目前為自由字串，導致既有 7 種歷程徽章樣式（`.submitted`／`.saved`／`.rejected`／`.consensus`／`.divergent`／`.overridden`／`.adjudicated`）僅有 3 種被渲染路徑使用，其餘為死碼。
...
- **理由必填**：審核退回、審核修改、爭議仲裁、標記員跳過四個動作一律必填 `reason`。**BREAKING**（標記員互動）：「跳過」由單鍵動作改為需填寫理由後方可送出。
...
- FR-049 提交紀錄定址／CONC-03：快照會顯著放大 `localStorage` 體積，且 `wsSubmissions` 目前為整包 read-modify-write，寫入競態下有 lost-update 風險，需於實作階段一併處理。
