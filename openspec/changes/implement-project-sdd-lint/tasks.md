...
- [x] 4.4 修改僅 `scripts/check-sdd.sh`。在既有 newline／TSV flows 前加入最小 NUL-safe preflight；不得修改 Red test、baseline、generator 或 `check-spec-artifacts.sh`。驗證 shell syntax、full harness、real lint、inventory check/tests、deterministic/no-write behavior，且不出現 retired `INVENTORY_FRESHNESS_UNVERIFIED`。 [@senior-devops]
