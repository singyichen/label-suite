...
- Final merge 後依 SC-007 只更新 Issue #375 已實際交付的七個 D items（canonical headings、STATUS/stage、Source-Verify、task exception、assignee/ownership、retired path/command、design inventory freshness）與指定 combined acceptance `CI 或本地單一命令可偵測 STATUS drift、retired path、規格必要段落與 inventory stale。`；inventory workstream C、baseline-zero cleanup 與所有其他 checkbox 保持原狀。
