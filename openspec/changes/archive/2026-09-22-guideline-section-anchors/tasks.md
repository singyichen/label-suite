...
- [x] 3.2 執行 gate 4：先逐條 grep 複驗本次新增的全部正典引用（FR／AC 編號、章節引用、檔案路徑、issue 編號、逐字引述之條文），再執行 `openspec archive guideline-section-anchors --yes`，並確認衍生檢視 diff 的移除行中 `#### Scenario` 計數為 0。 [@main]
  - 證據：archive 前已逐項定位 v6.14.0、FR-020D／FR-096、AC-5.5／AC-1.34 與三個原型檔案。`npx --yes @fission-ai/openspec@1.10.0 archive guideline-section-anchors --yes` → exit **0**，change 移至 `openspec/changes/archive/2026-09-22-guideline-section-anchors/`，derived view 更新 **1 modified requirement**。archive 後逐條複驗：FR／AC 均可在 v6.14.0 正典定位；`task-management/014-task-detail` FR-013 第 (6) 點可在 `specs/_archive/014-task-detail/spec.md` 定位；issue #596／#620／#834 經 GitHub GraphQL 確認存在且主旨相符；三個原型檔案均存在；需求子句逐句對照無遺漏。derived view diff 的移除行中 `#### Scenario` 計數為 **0**。
