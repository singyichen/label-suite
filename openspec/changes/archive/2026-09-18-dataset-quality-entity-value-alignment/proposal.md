...
同日維護者另裁定（issue #783 Q9）：「IAA 計算尚未結束不是 IAA 結果」的交叉引用寫進本 change，於 FR-039 第 1 點補一句，仍為 PATCH v3.0.1；並裁定 design.md Q1 延後（YAGNI）、Q2 的 CHECK 交由後端實作定案。
...
- **FR-039 第 1 點補一句釐清**（Q9）：IAA 仍在計算中或計算失敗時尚無 IAA 結果，不屬第 1 點的「α 未達門檻」；此時能否進入正式標記依正典 014 待 IAA 確認頁的計算狀態需求（`TrialRound.iaa_computation_status`）處理。第 1 點對 IAA 結果的非阻擋語意不變（design.md D4）。
...
**delta 形式說明**：FR-039 已在衍生檢視內，以 `## MODIFIED` 帶入全文，除新增的一句外逐字沿用，情境 AC-3.16 不變。FR-008 與 FR-025 不在 `openspec/specs/` 衍生檢視內（衍生檢視目前只收錄 FR-009L、FR-012L、FR-013、FR-024A、FR-035、FR-036、FR-039、FR-041、FR-042、FR-043），無法使用 `## MODIFIED`，因此以既有 ID 置於 `## ADDED Requirements`；gate 4 回寫正典時必須**原地改寫**，不得新增第二條同 ID 條文。
...
- `dataset/017-dataset-analysis-detail`：FR-008、FR-025 補述實體形狀；FR-039 第 1 點補一句交叉引用；關鍵實體 `SharedMetrics`、`AnnotatorRiskAssessment` 改寫。規格常數、FR-026、SC-003、SC-014 維持原文。

## Impact

**規格**

- 正典：`specs/dataset/017-dataset-analysis-detail/spec.md`（v3.0.0 → v3.0.1，**PATCH**）。判為 PATCH 的理由：修正同一規格內部的不一致，以既有常數段與 FR 為準，不新增、不移除任何 FR／AC／SC，任何既有畫面行為都不變；FR-039 的新增句只界定第 1 點的適用範圍，不新增 017 自己的行為（design.md D4）。
- 正典待改寫錨點（gate 4）：`:419` FR-008、`:458` FR-025、`:541` `SharedMetrics`、`:557` `AnnotatorRiskAssessment`、`:471` FR-039 第 1 點末尾、Changelog 新增 v3.0.1 列（現最新列在 `:646`）。
...
- 上游／下游：`dataset/016-dataset-analysis-list` 只消費任務層級 IAA 摘要，不讀這兩個實體。FR-039 的新增句引用 `task-management/014-task-detail` 的 `TrialRound.iaa_computation_status`，該欄位由姊妹 change `task-detail-iaa-precondition-and-override-scope` 新增，因此本 change 須在該 change 回寫正典 014 之後才 archive。
