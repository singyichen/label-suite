...
- **014 擁有設定與閘門**（014 側正典修改與實作已延後至 companion change，見 `deferred/`；涉及之 014 需求條目編號詳見 `deferred/014-task-detail-delta.md`）：兩份名冊的勾選、審核指派區塊的唯讀呈現、爭議池／例外池的計數列、例外池的清單與導頁、結案閘門。
...
- **[例外池不清空導致任務永遠結不了案]** 若任務未勾選仲裁者，爭議項會堆在爭議池、永不進例外池，`completed` 永遠被擋 → **緩解**：014 之發布閘門（companion change，見 `deferred/`）於發布確認顯示「未指定仲裁者將導致無法結案」警示（不阻擋發布，因為 `draft` 階段可能還沒決定人選）。
...
2. **014 發布閘門**（companion change，見 `deferred/`）：審核員 ≥ 1 才可發布（硬擋），仲裁者名冊為空僅警示不擋。可行性依據：`arbiter_ids` 屬 `OVERVIEW_EDITABLE_FIELDS`，發布後仍可編輯，爭議項堆積時維護者隨時能補人，死路可逃脫。
