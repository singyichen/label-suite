...
- [x] 0.1 於 `specs/STATUS.md` 將 `annotation-015` 的狀態由 spec-ready 更新為 change-open 並填入本 change 名稱。驗證：`scripts/check-sdd.sh` 不再回報 ACTIVE_CHANGE_STAGE （2026-09-04 補記）`specs/STATUS.md` 之 `annotation-015` 於 propose 期即已填入 `change-open` 與本 change 名稱，本次 archive 後由 3.7 接續更新為實作中狀態。 [@main]
...
- [x] 2.3 撰寫 Red 契約覆蓋差異比對與統計盒：新增 `design/prototype/tests/annotation/issue-581-span-diff-and-stats.spec.ts`——審核員更動一個 3 字實體的類型時差異項為 2 項而非 3 項，未更動時回報相同；統計盒顯示不帶前綴的標籤類型計數且不因實體橫跨多字元而重複計數。驗證：`PW_PORT=8922 corepack pnpm playwright test tests/annotation/issue-581-span-diff-and-stats.spec.ts` 全數失敗，失敗原因為比對仍為逐 token 位置且統計仍計帶前綴的 tag。實際證據：Red commit `9026e421`，4 failed / 0 passed（比對仍為逐 token 位置、統計仍計帶前綴的 tag） [@senior-qa]
- [x] 2.4 （Green）修改 `design/prototype/pages/annotation/annotation-workspace.data.js`：審核員模擬資料列的 `sequence_tagging` 答案改為帶 offset 的四鍵形狀，答案轉換函式的該型別分支與差異比對合併鍵改用起訖點加標籤。驗證：`PW_PORT=8922 corepack pnpm playwright test tests/annotation/issue-581-span-diff-and-stats.spec.ts` 全綠。實際證據：Green commit `47f01107`，4 passed / 0 failed；合併鍵定為 `start::end::label` 並沿用既有 `diffItemSets()`；每個 span 的顯示文字改由答案自帶的來源文字（`previewState.sequence_tagging.textKey`）切出，故轉換函式第三個 `opts` 參數（`sequenceTagsToPairs`）一併移除；`annotation-review-unit.spec.ts` 中改以位置比對的舊案例同步遷移為 span 集合比對，18 passed [@senior-frontend]
- [x] 2.5 撰寫 Red 契約覆蓋審核員操作與列表呈現：解除 `design/prototype/tests/annotation/annotation-workspace-reviewer.spec.ts` registry 迴圈中 `sequence_tagging` 案例的跳過標記並將其作答動作由點擊 Token 改為拖曳圈選，同時補上列表頁單列答案摘要顯示標記文字的斷言。驗證：`PW_PORT=8922 corepack pnpm playwright test tests/annotation/annotation-workspace-reviewer.spec.ts` 全數失敗，失敗原因為審核員直接修正路徑仍走 Token 網格。實際證據：Red commit `d7b01bbc`，1 failed / 32 passed；任務預測的「全數失敗」未成立——registry 迴圈的 `sequence_tagging` 案例在群組 1 已遷移的工作區上解除跳過後直接通過，真正為紅的是新增的列表頁單列答案摘要斷言（仍輸出 tag 而非標籤鍵） [@senior-qa]
- [x] 2.6 （Green）修改 `design/prototype/pages/annotation/annotation-list.html`：正式標記回合單列答案摘要的 `sequence_tagging` 分支改讀標籤鍵，並保持既有以文字鍵呈現標記內容的路徑不變。驗證：`PW_PORT=8922 corepack pnpm playwright test tests/annotation/annotation-workspace-reviewer.spec.ts` 全綠。實際證據：Green commit `479343fb`，33 passed / 0 failed（`formatCompactAnswer` 的該型別分支改讀 `span.label`） [@senior-frontend]
...
- [x] 3.1 更新 `specs/annotation/015-annotation-workspace/spec.md`：回寫本 change 的六項 MODIFIED 需求，版本由 5.0.0 升至 6.0.0（MAJOR：標記介面、提交 payload、CompactAnswer 形狀、差異比對鍵四項皆為 BREAKING），並新增對應 Changelog 條目。實際：六項 MODIFIED（FR-024A、FR-024A-1、FR-024A-2、FR-024A-3、FR-052、FR-024L）全數回寫為正典的單行條列體例，另做三處 delta 未涵蓋的一致性傳播——AC-2A.5、AC-3.12、AC-4.11；frontmatter 5.0.0 → 6.0.0 並新增 v6.0.0 Changelog 條目 [@main]
- [x] 3.2 更新 `specs/annotation/015-annotation-workspace/spec.md` 的規格常數與成功標準：`SEQ_MAJORITY_INVALID_BIO_FALLBACK` 標為本版正式廢止且 ID 不重用，SC-008 列舉的標記控制項由 Token 網格改為 span 圈選面，word-mode 分詞引擎選型的 Open Question 移除，上游依賴表對 013 的 tokenization 契約引用改為 span 契約。實際：常數區第 32 行 `SEQUENCE_TOKEN_UNITS` 更名為 `SPAN_SNAP_UNITS`、第 69 行 `SEQ_MAJORITY_INVALID_BIO_FALLBACK` 補記 v6.0.0 正式廢止且 ID 不重用；SC-008 控制項列舉「Token 網格」改為「span 圈選面」；Open Question（CKIP／Jieba／PyICU 選型）改記為匯出層 BIO 推導由 `dataset/017` 承接；上游依賴表 013 列改引 013 FR-003d-1 的 span 契約；另同步第 308 行假設段與第 631 行邊界情境（原以 ADR-031 後端權威 Token 邊界與數量對齊為前提）。殘留 `tagging_scheme`／`SEQUENCE_TOKEN_UNITS` 5 處皆為更名註記、已廢止 FR-035 的刪節線原文、移除清單與 Changelog [@main]
- [x] 3.3 措辭同步（Lightweight Path）：更新 `specs/dashboard/012-dashboard/spec.md` 的 `sequence_tagging` token 與 tag 措辭為 span 與標籤，PATCH 升版並補 Changelog。驗證：該檔不再出現 tagging_scheme 與 tokenization 字串。實際：`grep -c` 回 0，`sequence_tagging` 於 012 僅出現在 `OUTPUT_TYPE_KEYS` 列舉（第 33 行），無任何 token／tag／BIO 措辭可改，因此不做編輯、不升版——為無內容變更升 PATCH 只會製造雜訊 [@main]
- [x] 3.4 措辭同步（Lightweight Path）：更新 `specs/task-management/014-task-detail/spec.md` 的 `sequence_tagging` token 與 tag 措辭為 span 與標籤，PATCH 升版並補 Changelog。驗證：該檔不再出現 tagging_scheme 與 tokenization 字串。實際：唯一命中處為 014 Overview「標記設定」摘要成功標準舉例的「序列標註顯示標籤集/標記格式」，已改為「標籤集/選取吸附」；版本 2.11.1 → 2.11.2 並補 Changelog 條目；改後全檔 `tagging_scheme`／`tokenization` 僅剩該 Changelog 條目本身一處 [@main]
- [x] 3.5 執行 Source-Verify：逐一驗證衍生檢視中每個正典引用（FR 與 AC 編號、章節、檔案路徑、ADR 與 issue 與 PR 編號）皆可定位。驗證：`grep` 逐項命中，無漏無誤 （2026-09-04 完成）Source-Verify 通過：本 change 的 delta、tasks.md 與 proposal.md 所引用之 20 個檔案路徑全數存在（缺檔清單為空），含 issue #581 涉及之六份 spec、`scripts/gen-screen-inventory.mjs` 與 `docs/adr/033-openspec-change-workflow.md`；正典 `specs/annotation/015-annotation-workspace/spec.md` 逐 ID 命中 — FR-024A／FR-024A-1／FR-024A-2／FR-024A-3／FR-052／FR-024L／FR-035 各 1 次，SC-004／SC-004C／SC-004H／SC-008 各 1 次、SC-004R 2 次；AC-2A.5 2 次、AC-3.12 4 次、AC-4.11 5 次；上游 `specs/task-management/013-task-new/spec.md` 之 FR-003d-1 命中 1 次。 [@main]
- [x] 3.6 執行 archive 合併衍生檢視。驗證：`openspec archive seq-tagging-span-workspace` 後 `openspec validate --changes --no-interactive` 退出碼 0 （2026-09-04 完成）以 `/Users/mandychen/Library/pnpm/openspec`（v1.10.0）執行 `openspec archive seq-tagging-span-workspace --yes`，輸出 `annotation/015-annotation-workspace: update`、`~ 6 modified`、`Totals: + 0, ~ 6, - 0, → 0`，change 已改名為 `2026-09-04-seq-tagging-span-workspace`；隨後 `openspec validate --changes --no-interactive` 退出碼 0。踩雷兩則：（1）nvm 路徑下的 `openspec` 為 v1.4.1，只探測深度 1 的能力資料夾，會靜默回報 No deltas found 而 archive 出零更新，必須改用 pnpm 全域安裝的 v1.10.0；（2）本 change 的六條需求在衍生檢視原本不存在，MODIFIED 無前值可比對，先以正典 v5.0.0 原文建立基線（scenario 標題須與 delta 完全一致）後 archive 方能成功。 [@main]
- [x] 3.7 更新 `specs/STATUS.md` 將 `annotation-015` 標為 archived 並記錄 6.0.0。驗證：`scripts/check-sdd.sh` 回報 0 error （2026-09-04 完成）`specs/STATUS.md` 之 `annotation-015` 更新為 spec v6.0.0、分支 `feat/issue-581-span-preview`、記錄 change 已 archive，並新增 2026-09-04 變更紀錄列。狀態值採 `in-progress` 而非 `archived`：本表 `archived` 之定義為正典已移至 `specs/_archive/`，而本功能依 issue #578／#596 先例留在 `specs/annotation/`（`scripts/check-spec-artifacts.sh` 不認 `specs/_archive/`）。`scripts/check-sdd.sh` 回報 0 error。 [@main]
