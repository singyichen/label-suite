...
- [x] 1.3 執行 code/test gate：`node ~/.cache/node/corepack/v1/pnpm/12.3.4/bin/pnpm.mjs typecheck` 與 `node ~/.cache/node/corepack/v1/pnpm/12.3.4/bin/pnpm.mjs playwright test`（於 `design/prototype/`，並帶本 worktree 專屬 `PW_PORT`），兩者預期 exit `0`；typecheck 必須與 Playwright 分開記錄，兩者是獨立閘門。（rebase 前於 Green `819b4ad7`、`PW_PORT=8931`：`typecheck` exit 0；`playwright test` 全量 exit 0，`1703 passed`。rebase 到 `5f080421` 後於 `5598a01b` 重跑：`typecheck` exit 0；`playwright test` 全量 exit 1，`1729 passed／1 failed`——失敗者為與本變更無關之 `tests/dataset/dataset-analysis-list-filters-pagination.spec.ts:481`（debounce 時序斷言，本分支未動 `dataset/` 任何檔案），單獨以 `--repeat-each=3` 重跑 exit 0、`3 passed`，判定為既有 flaky） [@main]
