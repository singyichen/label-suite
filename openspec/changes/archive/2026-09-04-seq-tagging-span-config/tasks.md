...
- [ ] 3.2a 於 `docs/product/visual-overview/task-new-output-types.html` 依新契約改寫序列標註卡片：移除「標記單位」控制與 Token 網格，改以原始文本加 span 色條呈現。驗證：`/usr/bin/grep -c 'sequence-token-unit-control' docs/product/visual-overview/task-new-output-types.html` 回 0 [@senior-technical-writer]
- [ ] 3.2b 於 `design/prototype/tests/task-management/task-new-output-types-overview.spec.ts` 改寫序列標註卡片的斷言：以 span 色條取代標記單位控制與 Token 數量。驗證：`PW_PORT=8893 corepack pnpm playwright test tests/task-management/task-new-output-types-overview.spec.ts` 全綠 [@senior-qa]
