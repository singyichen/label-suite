...
- [x] 1.8 更新 `specs/annotation/015-annotation-workspace/spec.md`，完成 gate 4 回寫：版號 MINOR bump（新增 3 個 AC，未移除任何既有 FR／AC），Changelog 補一列，FR-093 本文接續 v6.17.0 追加「本版修訂（issue #956）」段落（含 archive 時把 delta 之 MUST 風格轉為正典既有 必須／不得 措辭）。delta 中未編號之新情境接續 US4（FR-093 對應使用者故事）現行最大 AC 編號續編。`specs/STATUS.md` 只動 `annotation-015` 一列。執行 `/opsx:archive`，依 `docs/sdd-workflow.md` §6.2 逐條 grep 驗證所有引用（FR-093、AC 新編號、版本號、issue #956/#921/#960/#868、`getAssignedReviewUnits()`、`isArbiterCandidate()`、`enumerateReviewUnits()`、`filterUnitsToAssigned()` 檔案行號）皆可定位。 [@main]
  - 版號 MINOR bump（初版）：6.25.0 → 6.27.0（新增 AC-4.72 ~ AC-4.74，未移除任何既有 FR／AC），commit `7b5884ad`。`specs/STATUS.md` 只動 `annotation-015` 一列。`openspec archive` 已執行，衍生檢視同步、change 已移至本目錄。Source-Verify 逐條 grep 驗證：`AC-4.72`（3 處）、`AC-4.73`（2 處）、`AC-4.74`（3 處）、`getAssignedReviewUnits`（6 處）、`isArbiterCandidate`（4 處）、`enumerateReviewUnits`（1 處）、`filterUnitsToAssigned`（1 處）、`isCurrentUnitAssigned`（3 處）、`v6.27.0`（5 處）、issue `#956`（6 處）、`#921`（6 處）、`#960`（1 處）、`#868`（15 處）皆存在；10 個引用之既有測試檔名逐一確認實際存在於 `design/prototype/tests/`。**此版號與 AC 編號已於 1.10 因與 #920 撞號而重編，見 1.10。**
- [x] 1.9 PR #999 CI 紅燈修正（真實迴歸，非測試位移）：`filterUnitsToAssigned()` 的仲裁豁免析取只認「目前爭議中」，仲裁送出後單位轉態即從左欄消失，破壞 issue #722 之進度計數器即時前進契約。TDD 先行：`senior-qa` 於 `issue-956-workspace-left-column-filter.spec.ts` 新增第 5 案例（commit `25ee3f76`，team lead 獨立複驗 4 passed/1 failed）→ `senior-frontend` 於 `filterUnitsToAssigned()` 新增第二個析取，重用既有 `isArbitrationSubmitted()`，並以 `isCurrentUnit()` 限定範圍（commit `dee4e99e`，避免無條件套用時復活任何歷史仲裁單位，team lead 獨立複驗 5 passed + 兩個既有回歸探針 25 passed + typecheck exit 0）。修正 `issue-722-arbiter-progress-counter.spec.ts` 之 `:74`／`:82` 位移斷言為 `0/1`、`1/1`（commit `d6c260df`）。team lead 獨立複驗完整迴歸時另發現二階位移：`issue-452-review-progress-subjects.spec.ts:126` 因本次修正從 3 變 4（目標單位本身即為受測身分已仲裁、當前開啟中之單位），已憑 `page.evaluate()` 查證後修正（commit `72475454`）。生成檔重生（commit `1dc53d13`）。完整重跑本次牽動之全部檔案（分兩批）→ 112 passed, 0 failed。已 push（`f1004f9b..1dc53d13`，未 rebase／未 force）。 [@main]
- [x] 1.10 AC 編號與版號重編（因與同波 #920 撞號，主 session 裁決）：main 現有最大 AC-4.x 為 AC-4.71（#921）；#920 亦基於同一起點各自配號到 AC-4.72 ~ AC-4.75，與本變更完全重疊。合併順序訂為 #920 → 本變更 → #992，故本變更之 AC-4.72／AC-4.73／AC-4.74（初版三個）與新增之仲裁送出後可見 AC（原暫編 AC-4.75）依序重編為 **AC-4.76、AC-4.77、AC-4.78、AC-4.79**；版號自 v6.27.0 改列 **v7.1.0**（接續 #920 之 MAJOR v7.0.0）。已改動：正典 frontmatter 版本欄、FR-093 兩處版本歷史括號、Changelog 該列版本欄與內文 AC 引用（並補一句版號說明記錄本次重編緣由）、本檔（`tasks.md`）本項與 1.8 證據列、`specs/STATUS.md` 之 `annotation-015` 一列。衍生檢視與 archived delta 之 FR-093 本文與 Scenario 皆未以文字引用 AC 編號（僅正典之編號清單引用），故兩檔僅需補上仲裁 sticky 之需求段落與 Scenario（已於 1.9 對應之 spec 補寫時一併處理），不需重新編號。重編後 `grep -rn "AC-4\.7[2345]"`（覆蓋正典／衍生檢視／archived delta／`proposal.md`／本檔／`design/prototype/`）僅命中正典 Changelog 內「版號說明」段落之歷史敘述（記錄原編號與撞號緣由，非殘留），其餘零命中。 [@main]
