...
| 屬性 | Token | 值 |
|------|-------|-----|
| 顏色 | `border-slate-200` | #E2E8F0 |
| 粗細 | `h-px` / `border-t` | 1px |

---

**變體：**

#### 1. 水平線（Horizontal Rule）

```html
<hr class="h-px bg-slate-200 border-0 my-6">
```

- 用於分隔頁面區塊（如 Card 內部、Form 區段）
- `my-6`（24px）為預設間距；若空間緊湊可改 `my-4`
...
- 僅用於登入頁 OAuth 與帳號登入之間的分隔
- 文字使用 `text-sm text-slate-400`（不使用 `text-ink`，降低視覺權重）

#### 3. 清單分隔（List Divider）

透過 Tailwind divide utilities 套用，不寫獨立 `<hr>`：
...
- `divide-slate-100` 比 `divide-slate-200` 更淡，適合密集清單

**When NOT to use：**
- 不要用 Divider 分隔過多區塊（超過 3 個相鄰分隔線通常代表需要重新考慮佈局）
- 不要用 Divider 取代 spacing；先嘗試加大 padding/margin，只有在視覺分組不清楚時才加 Divider

---

### List（Activity List）

**用途：** 顯示一組同質性項目，每筆資料格式一致（如活動紀錄、任務清單）。

**Token：**

| 屬性 | 值 |
|------|----|
| 分隔線 | `divide-y divide-slate-100` |
| Row padding | `py-4` |
| 左側主文 | `text-sm font-medium text-ink` |
| 右側次文 | `text-sm text-slate-400` |
...
<ul class="divide-y divide-slate-100">
  <li class="flex items-center justify-between py-4 hover:bg-slate-50 px-1 rounded">
    <span class="text-sm font-medium text-ink">任務名稱</span>
    <span class="text-sm text-slate-400">2024-01-15</span>
  </li>
  <li class="flex items-center justify-between py-4 hover:bg-slate-50 px-1 rounded">
    <span class="text-sm font-medium text-ink">任務名稱</span>
    <span class="text-sm text-slate-400 tabular-nums">92 分</span>
  </li>
</ul>
```
...
| 類型 | 樣式 |
|------|------|
| 日期 | `text-sm text-slate-400` |
| 分數 / 數字 | `text-sm text-slate-400 tabular-nums` |
| Status Badge | 嵌入 `<span>` Badge component |
...
**When NOT to use：**
- 資料需要排序、篩選、多欄比較 → 改用 **Table**
- 項目需要展開更多細節 → 考慮 Accordion 或獨立頁面
