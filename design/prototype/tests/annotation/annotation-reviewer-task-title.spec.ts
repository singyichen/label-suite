...
const REVIEWER_TASK_TITLE_CASES = [
  {
    name: 'classification',
...
for (const taskCase of REVIEWER_TASK_TITLE_CASES) {
  test(`reviewer workspace uses annotator task title for ${taskCase.name}`, async ({ page }) => {
    ...
    await expect(page.locator('#reviewCardTitle')).toHaveText(taskCase.expectedTitle);
    await expect(page.locator('#reviewerRoleSmall')).toBeHidden();
  });
}
