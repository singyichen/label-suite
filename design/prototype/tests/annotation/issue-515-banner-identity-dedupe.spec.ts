 ...
 * sits in the banner whose job is the REVIEW MODEL (run type, state, state
 * track) -- not identity. This spec removes it and pins that the two
 * remaining answers still hold, so the information is deduplicated rather
 * than lost.
 *
 * issue #596 then removed the finalize-threshold chip as well (AC-4.37):
 * one reviewer owns a unit, so there is no quorum to count against. The
 * banner therefore keeps exactly ONE chip -- run type. Counting chips is
 * still the point -- an assertion that merely greps the banner for an
 * account name would pass if the identity chip came back carrying
 * different text.
 *
...
test.describe('脈絡橫幅不再重複身分資訊 (issue #515 ①)', () => {
  test('多標記員樣本：橫幅只剩 run type 一個 chip', async ({ page }) => {
    await page.goto(T014_UNIT);
    await dismissGuidelineModal(page);

    /* issue #596 re-anchor: the identity chip this spec removed is still
       gone, and the threshold chip that used to sit beside it went with the
       reviewer quorum (AC-4.37). Counting the survivors keeps the original
       subject -- no chip may reintroduce identity under another name. */
...
test.describe('橫幅其餘內容不受影響 (issue #515 ①)', () => {
  test('run type 徽章、狀態 pill 與狀態軌都還在', async ({ page }) => {
    await page.goto(T014_UNIT);
    await dismissGuidelineModal(page);
