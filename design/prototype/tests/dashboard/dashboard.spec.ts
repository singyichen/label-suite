...
test.describe('Dashboard page — scenario rendering', () => {
  ...
  test('reviewer view shows pending review panel and start-review action', async ({ page }) => {
    ...
    /* issue #501: the per-task summary is derived from live review-unit
       state, so these two cards read the formula's output rather than the
...
test.describe('Dashboard page — language toggle', () => {
  ...
  test('reviewer workflow card copy reflects the independent-review model in both locales', async ({ page }) => {
    ...
    await expect(page.locator('#stepReviewer2Desc')).toHaveText('通過、修正或無法裁決；歧異交付仲裁與例外池');
    ...
    await expect(page.locator('#stepReviewer2Desc')).toHaveText('Approve, modify, or cannot adjudicate; disputes go to arbitration and the exception pool');
  });
