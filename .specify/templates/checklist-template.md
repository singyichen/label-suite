...
- [ ] CHK032 驗收情境的 Given 是否包含具體數值或明確狀態（例：「$100,000 帳戶」、「狀態 = Active」），而非模糊指稱（「某個帳戶」）？[Clarity, Spec §使用者情境與測試]
...
- [ ] CHK010 涉及 UI 時，每個畫面是否列舉六種狀態（空白 / 載入中 / 正常 / 成功 / 失敗 / 邊界），不適用者是否標記原因？[Coverage, Spec §畫面狀態規格]
- [ ] CHK033 每個功能規則的驗收情境是否附有至少 2 個邊界情境，優先從以下三類挑選：規則同時觸發的優先級、時間 / 時區邊界、資料不完整時的降級行為？[Edge Case, Spec §邊界情況]
...
- [ ] CHK027 若前端受影響，規格是否定義 async 操作的 loading、error、empty 與 cleanup 行為，而非僅描述成功情境？[Frontend Constitution XIII, Spec §邊界情況]
- [ ] CHK028 若功能新增 API 端點，規格是否要求文件化 enum 值、nullable 欄位、error response schema 與 pagination shape？[Backend Constitution III; Frontend Constitution XIV, Spec §需求規格]
- [ ] CHK029 若功能涉及核心實體狀態（task、annotation、batch、export），規格是否定義合法轉換清單與非法轉換的拒絕行為？[Backend Constitution XII, Spec §使用者情境與測試]
- [ ] CHK030 規格或測試計畫是否明確說明測試資料策略（使用合成資料或已核可的清洗資料集，不使用 production 資料）？[Testing Constitution XI, Gap]
