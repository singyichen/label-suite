...
| 004 | 個人設定（資料編輯 + 修改密碼）| `profile` | account | ★☆☆☆☆ | ⬜ 待重做 |
| 005 | 使用者列表與管理 | `user-management` | admin | ★★☆☆☆ | ⬜ 待做 |
| 006 | 角色權限設定 | `role-settings` | admin | ★★☆☆☆ | ⬜ 待做 |
| 007 | 平台成員列表（搜尋、啟用/停用、待指派）| `annotator-list` | annotator-management | ★★☆☆☆ | ⬜ 待做 |
| 008 | 工時紀錄 | `work-log` | annotator-management | ★★☆☆☆ | ⬜ 待做 |
| 009 | 任務列表（搜尋、篩選、空狀態）| `task-list` | task-management | ★★☆☆☆ | ⬜ 待做 |
| 010 | 儀表板 — Annotator | `dashboard` | dashboard | ★★☆☆☆ | ⬜ 待做 |
| 011 | 儀表板 — Super Admin | `dashboard` | dashboard | ★★★☆☆ | ⬜ 待做 |
| 012 | 儀表板 — Reviewer（任務角色）| `dashboard` | dashboard | ★★★☆☆ | ⬜ 待做 |
| 013 | 儀表板 — Project Leader（任務角色）| `dashboard` | dashboard | ★★★☆☆ | ⬜ 待做 |
| 014 | 任務詳情 — 資訊顯示與狀態 | `task-detail` | task-management | ★★★☆☆ | ⬜ 待做 |
| 015 | 任務詳情 — 成員邀請與 Dry Run 發布 | `task-detail` | task-management | ★★★★☆ | ⬜ 待做 |
| 016 | 任務詳情 — Official Run 發布與匯出 | `task-detail` | task-management | ★★★★☆ | ⬜ 待做 |
| 017 | 任務建立 Step 1 — 基本資料與任務類型選擇 | `task-new` | task-management | ★★☆☆☆ | ⬜ 待做 |
| 018 | 任務建立 Step 3 — 標記說明與強制顯示設定 | `task-new` | task-management | ★★☆☆☆ | ⬜ 待做 |
| 019 | Config Builder — 分類任務（Classification）| `task-new` | task-management | ★★★☆☆ | ⬜ 待做 |
| 020 | Config Builder — 評分/回歸任務（Scoring）| `task-new` | task-management | ★★★☆☆ | ⬜ 待做 |
| 021 | Config Builder — 句對任務（Sentence Pair）| `task-new` | task-management | ★★★☆☆ | ⬜ 待做 |
| 022 | Config Builder — 序列標記（NER）| `task-new` | task-management | ★★★★☆ | ⬜ 待做 |
| 023 | Config Builder — 關係抽取（Relation Extraction）| `task-new` | task-management | ★★★★☆ | ⬜ 待做 |
| 024 | Config Builder — Code 模式與範本快速套用 | `task-new` | task-management | ★★★☆☆ | ⬜ 待做 |
| 025 | 標記作業 — 分類任務 | `annotation-workspace` | annotation | ★★★☆☆ | ⬜ 待做 |
| 026 | 標記作業 — 評分/回歸任務 | `annotation-workspace` | annotation | ★★★☆☆ | ⬜ 待做 |
| 027 | 標記作業 — 句對任務 | `annotation-workspace` | annotation | ★★★☆☆ | ⬜ 待做 |
| 028 | 標記作業 — 序列標記（NER）| `annotation-workspace` | annotation | ★★★★☆ | ⬜ 待做 |
| 029 | 標記作業 — 關係抽取 | `annotation-workspace` | annotation | ★★★★★ | ⬜ 待做 |
| 030 | 標記審查（任務角色：reviewer）| `annotation-workspace` | annotation | ★★★★☆ | ⬜ 待做 |
| 031 | 統計總覽 — 共用基礎指標（Sentence / Token / 完成率）| `dataset-stats` | dataset | ★★★☆☆ | ⬜ 待做 |
| 032 | 統計總覽 — 分類任務（含句對分類模式）| `dataset-stats` | dataset | ★★★★☆ | ⬜ 待做 |
| 033 | 統計總覽 — 評分/回歸任務（含句對評分模式）| `dataset-stats` | dataset | ★★★★☆ | ⬜ 待做 |
| 034 | 統計總覽 — 序列標記（NER）| `dataset-stats` | dataset | ★★★★☆ | ⬜ 待做 |
| 035 | 統計總覽 — 關係抽取 | `dataset-stats` | dataset | ★★★★☆ | ⬜ 待做 |
| 036 | 品質監控 — 異常偵測與標記員速度統計 | `dataset-quality` | dataset | ★★★☆☆ | ⬜ 待做 |
| 037 | IAA — 分類任務（含句對分類模式）| `dataset-quality` | dataset | ★★★★☆ | ⬜ 待做 |
| 038 | IAA — 評分/回歸任務（含句對評分模式）| `dataset-quality` | dataset | ★★★★☆ | ⬜ 待做 |
| 039 | IAA — 序列標記（NER）| `dataset-quality` | dataset | ★★★★★ | ⬜ 待做 |
| 040 | IAA — 關係抽取 | `dataset-quality` | dataset | ★★★★★ | ⬜ 待做 |
