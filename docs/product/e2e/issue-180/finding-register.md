...
| N-05 | annotation-results／export 吃 `T001` seed fallback，journey 產生的資料進不了 PL 面板與匯出 | `task-detail.html:7815` | **已建單 [#284](https://github.com/singyichen/label-suite/issues/284)**（Bug）：XROLE-19/22 曾以雙斷言繞行；**已修復**（spec 014 v2.10.1，`fix/task-detail-results-fallback`）：`getAnnotationResultsData()` 查無登錄改回傳空集合（面板空狀態、匯出零筆），T009／T012 明確登錄，XROLE-19/22 斷言改為誠實空狀態（journey 即時資料入面板仍為既有 gap） |
