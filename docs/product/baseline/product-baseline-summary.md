...
Label Suite 是一套**可配置、通用型 NLP 標注與自動評估平台**，作為碩士論文 Demo Paper 的研究成果展示。

核心目標：
- 在**不修改核心程式碼**的前提下，支援多種 NLP 任務類型
- 提供從任務建立、成員協作、標注執行、審核到品質分析的完整流程
- 確保標注評估結果具備**公平性、可重現性與可追溯性**
...
| 系統超級管理員 | `super_admin` | 負責平台使用者管理與系統權限設定 |

### 任務角色（任務層級）

| 角色 | 識別碼 | 說明 |
|------|--------|------|
| 計畫負責人 | `project_leader` | 建立任務、設定流程、指派成員、啟動 Dry Run / Official Run、匯出結果 |
| 審核員 | `reviewer` | 審查標注結果、協助產出標準答案、查看品質報告 |
| 標記員 | `annotator` | 執行標注、查看個人進度與工時 |
...
5. 達標後發布 Official Run，分配正式資料進行標注
