...
- Supported languages: `zh-TW` (default fallback) and `en`. The backend parses the `Accept-Language` header as follows: split the header value by `,` to obtain language ranges; for each range, extract the q-value (default `1.0` when absent) and sort ranges by descending q-value, using original header order as the tie-breaker; for each range in preference order, use the primary subtag (e.g. `en-US` → `en`) and replace hyphens with underscores (e.g. `zh-TW` → `zh_TW`); return the first language code that matches a supported language. Fall back to `zh-TW` if the header is absent, empty, or no range matches a supported language.
