...
- Supported languages: `zh-TW` (default fallback) and `en`. The backend parses the `Accept-Language` header as follows: split the header value by `,` to obtain language ranges; for each range, extract the q-value (default `1.0` when absent) and ignore ranges with `q <= 0`; sort the remaining ranges by descending q-value, using original header order as the tie-breaker; for each range in preference order, first try to match the full range with hyphens replaced by underscores (e.g. `zh-TW` → `zh_TW`), and if no match is found, fall back to matching the primary subtag (e.g. `en-US` → `en`); return the first language code that matches a supported language. Fall back to `zh-TW` if the header is absent, empty, no positive-q ranges remain, or no range matches a supported language.
