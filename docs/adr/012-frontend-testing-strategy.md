...
**Status**: Accepted
**Date**: 2026-04-03
**Superseded in part by**: [ADR-033](033-formal-e2e-directory-location.md) — every `frontend/tests/` path in this ADR becomes root `e2e/`, and the fixture `storageState:` literals become `shared/fixtures/.auth/*.json`. The two-layer strategy, the Vitest/RTL component layer, the five role fixtures, the Page Object Model layout, and the E2E coverage matrix rows are all unchanged.
...
> **Superseded in part by [ADR-033](033-formal-e2e-directory-location.md)**: the E2E row's location is now root `e2e/[module]/`. The component row is unchanged.
