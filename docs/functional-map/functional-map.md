...
>
> **版本：** v2（2026-04-02）— 依 [IA v3](../ia/information-architecture.md) 同步更新
...
flowchart LR
  ...
  nTN --> nTNTYPE["任務類型"]
  nTNTYPE --> nTT1["單句任務（分類 / 評分）"]
  nTNTYPE --> nTT2["句對任務（相似度 / 蘊含）"]
  nTNTYPE --> nTT3["序列標記（NER / 詞性標記）"]
  nTNTYPE --> nTT4["關係抽取（Entity + Relation + Triple）"]
  nTNTYPE --> nTT5["生成式標記（人工撰寫 / 評分）"]
  nTASK --> nTD["任務詳情頁"]
