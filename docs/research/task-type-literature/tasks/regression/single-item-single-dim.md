...
主要挑戰：
1. **尺度錨定（Scale Anchoring）**：不同標記者對「4 分」的直覺可能不同
2. **分佈偏移（Distribution Shift）**：某些標記者習慣性偏高或偏低
3. **量表壓縮（Range Restriction）**：標記者只使用量表中段，導致分數變異過小
...
| 指標 | 適用情境 | 說明 |
|------|----------|------|
| **ICC（2,1）絕對一致** | 多位標記者、關注分數是否相同 | 要求標記者不只趨勢一致，絕對數值也需接近；為此任務首選 |
| **ICC（2,1）一致性** | 允許系統性偏移 | 只要求相對趨勢一致，允許標記者有固定偏高或偏低傾向 |
| **Pearson Correlation** | 兩位標記者、連續分數 | 衡量線性同向變動，但不捕捉絕對數值差異 |
| **Spearman Correlation** | 序位比絕對值重要 | 衡量分數排序是否一致，適合離散量表（如 1–5 分）|
| **Mean Absolute Difference（MAD）** | 快速監控 | 直觀反映平均分歧幅度，適合即時品質監控 |

#### ICC 類型選擇指引

ICC(1,1)：標記者視為隨機抽樣，無法泛化到新標記者 → 較少使用
ICC(2,1)：標記者視為固定對象，分析個別差異      → 建議使用
ICC(3,1)：只關心相對排序，忽略系統性偏移        → 適合序位任務
絕對一致（Absolute Agreement）vs 一致性（Consistency）：
絕對一致：標記者 A 給 3.0、B 給 4.0 → 視為不一致
一致性：  標記者 A 給 3.0、B 給 4.0 → 若趨勢相同則視為一致
...
| **Per-annotator Score SD** | 各標記者的分數標準差，偵測是否只使用量表中段（壓縮效應）|
| **Per-annotator Score Distribution** | 分數的直方圖，識別雙峰分佈或異常集中現象 |
| **Annotator Bias（偏移量）** | 個別標記者與所有標記者平均的系統性差距 |
...
| **Anchor Consistency Rate** | 若有錨定範例（Anchor Examples），標記者在這些樣本的一致率 |
...
| 時間點 | 動作 |
|--------|------|
| 標記開始前 | Pilot round → 確認錨點定義一致；檢查各標記者分佈是否已出現偏移 |
| 標記進行中 | 監控 Per-annotator Mean Score 與 MAD，即時偵測分佈漂移 |
| 標記完成後 | 計算 ICC（絕對一致）；對 Annotator Bias 過大者進行校正或重新標記 |
