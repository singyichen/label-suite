...
| 新增複雜度 | 說明 |
|-----------|------|
| **維度獨立性假設** | 每個維度應衡量不同屬性，但標注者可能受其他維度分數影響（光環效應）|
| **維度間相關性檢驗** | 若兩維度高度相關，代表標注者無法有效區分，需重新定義或合併 |
...
範例（三維品質評估）：
  標注者 A：{clarity: 4, relevance: 3, completeness: 2}
  標注者 B：{clarity: 4, relevance: 4, completeness: 4}
...
| **Per-dimension Annotator Bias** | 各維度的標注者系統性偏移，分維度追蹤 |
| **維度間相關矩陣（Dimension Correlation Matrix）** | 所有標注者分數的維度兩兩 Pearson 相關，高相關（r > 0.80）代表維度定義重疊 |
...
| **維度間評分一致性** | 同一標注者在不同維度的給分是否過度相似（暗示光環效應）|
...
- **Per-annotator Per-dimension 散佈圖矩陣**：
  每對維度一張散佈圖，觀察標注者間的一致程度與異常點
- **High-Disagreement 樣本向量圖**：
  視覺化分歧最大的樣本，呈現各標注者在每個維度上的分數差異
...
| 時間點 | 動作 |
|--------|------|
| 標注開始前 | Pilot round → 逐維確認錨點定義一致；初步計算維度間相關矩陣，確認維度可被有效區分 |
| 標注進行中 | 監控 Per-dimension MAD 與 Per-annotator Bias，偵測特定維度的系統性漂移 |
| 標注完成後 | 完整計算維度間相關矩陣；ICC < 0.60 的維度優先修訂；相關係數 > 0.80 的維度評估是否合併 |
