...
> 針對 Label Studio（參考基準）與 Label Suite 進行多維度比較，涵蓋部署方式、任務支援、任務協作、資料集統計、使用者體驗與技術架構，用於定義 Label Suite 在 Demo Paper 中的貢獻與差異化定位。
...
|---|---|---|
| **類型** | 通用型資料標記平台 | 具任務協作與資料集分析的 NLP 專用標記平台 |
| **目標使用者** | 企業、ML 工程師、資料團隊 | 系統管理員、實驗室研究生、兼職工讀生標記員 |
| **主要使用情境** | 多模態標記（圖片、音訊、影片、文字） | 文字 NLP 標記 + 任務協作 + 資料集分析 |
| **授權模式** | Apache 2.0（開源） | Research Preview 聲明（保留所有權利） |
...
| 比較維度 | Label Studio | Label Suite |
|---|---|---|
| **帳號管理** | 基本使用者帳號 | 系統帳號由 admin 管理，任務權限由任務成員角色管理 |
| **任務角色** | 依專案設定，需額外配置 | `project_leader` / `reviewer` / `annotator` 任務層級角色 |
| **成員指派** | 需在專案設定中處理 | 任務詳情內完成成員加入與角色指派 |
| **工作量追蹤** | 依專案與匯出資料自行整理 | 任務詳情內彙整進度、標記結果與工時/工作量紀錄 |
| **稽核性** | 需依專案配置與匯出資料補強 | 任務活動、角色與標記結果可在任務脈絡追蹤 |
...
| **學生標記員工作流程** | ❌ | ✅（任務內進度、指派與品質回饋整合） |
| **社群複用（開源）** | ✅（大型社群、外掛生態） | ❌（Research Preview 階段，原始碼僅供檢視與評估，尚未開放複用） |
...
2. **任務協作流程整合** — 將成員指派、進度追蹤與品質回饋放在同一任務脈絡
