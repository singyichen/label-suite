...
if rg -n '/ui-ux-pro-max.*→|npm (test|run)|Relationship to `frontend/tests/`|checks cross-spec consistency' .claude/skills/sdd-workflow/SKILL.md; then exit 1; fi
git diff --check
```
...
if rg -n 'eight principles|/opsx:verify.*or.*openspec validate' openspec/config.yaml; then exit 1; fi
git diff --check
```
...
if rg -n 'opsx:verify.*or.*openspec validate|senior-qa.*frontend/tests' AGENTS.md; then exit 1; fi
git diff --check
```
...
if rg -n 'eight core principles|opsx:verify.*or.*openspec validate' CLAUDE.md; then exit 1; fi
git diff --check
```
...
if rg -n 'frontend/tests|/speckit.checklist|opsx:verify.*or.*openspec validate' .claude/agents/team-lead.md; then exit 1; fi
git diff --check
```
...
if rg -n 'frontend/tests|/speckit.checklist|opsx:verify.*or.*openspec validate' .claude/commands/agent-team.md; then exit 1; fi
git diff --check
```
...
if rg -n 'npm (test|run)|eight principles|8 principles|八原則|/speckit.checklist|opsx:verify.*or.*openspec validate' \
  AGENTS.md CLAUDE.md docs/sdd-workflow.md .claude/skills/sdd-workflow/SKILL.md \
