...
**AC ID 規則**：格式為 `AC-<故事編號>.<序號>`（例：`AC-2.3` = 故事 2 第 3 條）。ID 一經發布即穩定——後續插入新情境時沿用字尾字母（例：`AC-1.2A`），不得重編既有 ID（比照 `FR-004A` 慣例）。故事編號同樣穩定：編號是身分不是位置——新增故事取下一個未用編號、不重編既有故事；顯示順序可依優先級調整，但 AC ID 跟隨故事編號而非顯示位置。
