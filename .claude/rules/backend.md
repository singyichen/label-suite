...
- Read the `Accept-Language` header on every request to select the language; parse comma-separated language ranges in order: strip quality values (`;q=…`), use the primary subtag (e.g. `en-US` → `en`), replace hyphens with underscores (e.g. `zh-TW` → `zh_TW`), and return the first supported language; fall back to `zh-TW` if the header is absent, empty, or no range matches.
