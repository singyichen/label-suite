...
- Read the `Accept-Language` header on every request to select the language; normalize the value before matching (strip quality values like `;q=…`, use the primary subtag, replace hyphens with underscores to match directory names — e.g. `zh-TW` → `zh_TW`, `en-US` → `en`); fall back to `zh-TW` if the header is absent or unsupported.
